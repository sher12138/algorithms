...
class AABoxKDTree2dNode {
 public:
  ...
  AABoxKDTree2dNode(const std::vector<ObjectPtr> &objects,
      ...
      : depth_(depth) {
    if (objects.empty()) {
      LOG_ERROR("objects.empty(), crash error!");
      return;
    }
 ...
 private:
  ...
  void ComputeBoundary(const std::vector<ObjectPtr> &objects) {
    ...
    if (std::isinf(max_x_) || std::isinf(max_y_) || std::isinf(min_x_) ||
        std::isinf(min_y_)) {
      LOG_ERROR("the provided object box size is infinity, crash error");
      return;
    }
  }
 ...
 private:
  ...
  // TODO(wwl) : I think enum class may be better
  enum Partition {
    PARTITION_X = 1,
    PARTITION_Y = 2,
  };
