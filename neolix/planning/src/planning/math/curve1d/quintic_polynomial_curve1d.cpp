...
std::vector<double> QuinticPolynomialCurve1d::PolyCoef() const {
  std::vector<double> ans(coef_.size());
  ans[0] = coef_[0], ans[1] = coef_[1], ans[2] = coef_[2];
  ans[3] = coef_[3], ans[4] = coef_[4], ans[5] = coef_[5];
  return ans;
}
