...
bool FemPosOsqpSolver::Solve() {
  // Sanity Check
  if (ref_points_.empty()) {
    LOG_ERROR("reference points empty, solver early terminates");
    return false;
  }

  if (ref_points_.size() != bounds_around_refs_.size()) {
    LOG_ERROR("ref_points and bounds size not equal, solver early terminates");
    return false;
  }

  if (ref_points_.size() < 3) {
    LOG_ERROR("ref_points size smaller than 3, solver early terminates");
    return false;
  }

  if (ref_points_.size() > std::numeric_limits<int>::max()) {
    LOG_ERROR("ref_points size too large, solver early terminates");
    return false;
  }
  ...
  if (res == false || work == nullptr || work->solution == nullptr) {
    LOG_ERROR("Failed to find solution.");
    // Cleanup
    osqp_cleanup(work);
    c_free(data->A);
    c_free(data->P);
...
void FemPosOsqpSolver::CalculateKernel(std::vector<c_float>* P_data,
                                       std::vector<c_int>* P_indices,
                                       std::vector<c_int>* P_indptr) {
  if (num_of_variables_ < 4) {
    LOG_ERROR("num_of_variables_ [{}]< 4", num_of_variables_);
    return;
  }
  ...
  if (col_num != num_of_variables_) {
    LOG_ERROR("col_num [{}] != num_of_variables_ [{}]", col_num,
              num_of_variables_);
    return;
  }
...
void FemPosOsqpSolver::CalculateOffset(std::vector<c_float>* q) {
  for (std::size_t i = 0; i < num_of_points_; ++i) {
...
void FemPosOsqpSolver::CalculateAffineConstraint(
...
void FemPosOsqpSolver::set_ref_points(
    const std::vector<std::pair<double, double>>& ref_points) {
  ref_points_ = ref_points;
}

void FemPosOsqpSolver::set_bounds_around_refs(
    const std::vector<double>& bounds_around_refs) {
  bounds_around_refs_ = bounds_around_refs;
}

void FemPosOsqpSolver::set_weight_fem_pos_deviation(
    const double weight_fem_pos_deviation) {
  weight_fem_pos_deviation_ = weight_fem_pos_deviation;
}

void FemPosOsqpSolver::set_weight_path_length(const double weight_path_length) {
  weight_path_length_ = weight_path_length;
}

void FemPosOsqpSolver::set_weight_ref_deviation(
    const double weight_ref_deviation) {
  weight_ref_deviation_ = weight_ref_deviation;
}

void FemPosOsqpSolver::set_max_iter(const int max_iter) {
  max_iter_ = max_iter;
}

void FemPosOsqpSolver::set_time_limit(const double time_limit) {
  time_limit_ = time_limit;
}

void FemPosOsqpSolver::set_verbose(const bool verbose) { verbose_ = verbose; }

void FemPosOsqpSolver::set_scaled_termination(const bool scaled_termination) {
  scaled_termination_ = scaled_termination;
}

void FemPosOsqpSolver::set_warm_start(const bool warm_start) {
  warm_start_ = warm_start;
}

const std::vector<double>& FemPosOsqpSolver::opt_x() const { return x_; }

const std::vector<double>& FemPosOsqpSolver::opt_y() const { return y_; }

void FemPosOsqpSolver::SetPrimalWarmStart(
    std::vector<c_float>* primal_warm_start) {
  if (ref_points_.size() != num_of_points_) {
    LOG_ERROR("ref_points_.size({}) != num_of_points_ {}", ref_points_.size(),
              num_of_points_);
    return;
  }
...
bool FemPosOsqpSolver::OptimizeWithOsqp(
  ...
  *work = osqp_setup(data, settings);
  ...
  if (status < 0) {
    LOG_ERROR("failed optimization status: {}", (*work)->info->status);
    return false;
  }
