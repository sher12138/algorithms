#include "backup_path_hpipm_solver.h"
...
BackupPathHpipmSolver::BackupPathHpipmSolver(const std::string &solver_name,
                                             const std::size_t n,
...
void BackupPathHpipmSolver::SetDynamics(
    const BackupPath::State &x0, std::vector<BackupPath::Stage> &stages) {
  b0_ = (stages[0].line_model.A * BackupPath::StateToVector(x0) +
...
void BackupPathHpipmSolver::SetCost(std::vector<BackupPath::Stage> &stages) {
  for (int i = 0; i <= N_; i++) {
...
void BackupPathHpipmSolver::SetBounds(std::vector<BackupPath::Stage> &stages) {
  nbu_[0] = 0;
  ...
  for (int j = 0; j < N_U_; j++) {
    if (stages[0].lower_bounds_u(j) > -BackupPath::INF &&
        stages[0].upper_bounds_u(j) < BackupPath::INF) {
      nbu_[0]++;
  ...
  for (int i = 1; i <= N_; i++) {
    ...
    nbu_[i] = 0;
    for (int j = 0; j < N_U_; j++) {
      if (stages[i].lower_bounds_u(j) > -BackupPath::INF &&
          stages[i].upper_bounds_u(j) < BackupPath::INF) {
        nbu_[i]++;
    ...
    for (int j = 0; j < N_X_; j++) {
      if (stages[i].lower_bounds_x(j) > -BackupPath::INF &&
          stages[i].upper_bounds_x(j) < BackupPath::INF) {
        nbx_[i]++;
...
void BackupPathHpipmSolver::SetPolytopicConstraints(
...
void BackupPathHpipmSolver::SetSoftConstraints(
    std::vector<BackupPath::Stage> &stages) {
...
std::vector<BackupPath::OptVariables> BackupPathHpipmSolver::SolveMPC(
    const BackupPath::State &x0, std::vector<BackupPath::Stage> &stages,
    int *status) {
  ...
  std::vector<BackupPath::OptVariables> opt_solution = Solve(status);
  opt_solution[0].xk = x0;

  return opt_solution;
}

std::vector<BackupPath::OptVariables> BackupPathHpipmSolver::Solve(
    int *status) {
  // ocp qp dim
  ...
  std::vector<BackupPath::OptVariables> optimal_solution;
  ...
  for (int i = 0; i < N_; i++) {
    ...
    optimal_solution[i].uk = BackupPath::ArrayToControl(u);
  }
  ...
  for (int i = 1; i <= N_; i++) {
    ...
    optimal_solution[i].xk = BackupPath::ArrayToState(x);
  }
