...
#include "src/planning/task/optimizers/motorway_speed_optimizer_for_iter/motorway_speed_model_for_iter.h"
...
class MotorwaySpeedHpipmSolverForIter {
 public:
  MotorwaySpeedHpipmSolverForIter(const std::string &solver_name,
                                  const std::size_t n, const std::size_t n_x,
                                  const std::size_t n_u);

  ~MotorwaySpeedHpipmSolverForIter() = default;

  std::vector<MotorwaySpeedForIter::OptVariables> SolveMPC(
      const MotorwaySpeedForIter::State &x0,
      std::vector<MotorwaySpeedForIter::Stage> &stages, int *status);

 private:
  void SetDynamics(const MotorwaySpeedForIter::State &x0,
                   std::vector<MotorwaySpeedForIter::Stage> &stages);

  void SetCost(std::vector<MotorwaySpeedForIter::Stage> &stages);

  void SetBounds(std::vector<MotorwaySpeedForIter::Stage> &stages);

  void SetPolytopicConstraints(
      std::vector<MotorwaySpeedForIter::Stage> &stages);

  void SetSoftConstraints(std::vector<MotorwaySpeedForIter::Stage> &stages);

  std::vector<MotorwaySpeedForIter::OptVariables> Solve(int *status);
 ...
 private:
  ...
  std::vector<MotorwaySpeedForIter::HpipmBound> hpipm_bounds_;
  Eigen::MatrixXd b0_;
};
