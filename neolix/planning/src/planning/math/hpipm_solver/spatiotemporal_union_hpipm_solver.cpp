#include "spatiotemporal_union_hpipm_solver.h"
...
SpatiotemporalUnionHpipmSolver::SpatiotemporalUnionHpipmSolver(
...
void SpatiotemporalUnionHpipmSolver::SetDynamics(
    const SpatiotemporalUnion::State &x0,
    std::vector<SpatiotemporalUnion::Stage> &stages) {
  b0_ = (stages[0].line_model.A * SpatiotemporalUnion::StateToVector(x0) +
...
void SpatiotemporalUnionHpipmSolver::SetCost(
    std::vector<SpatiotemporalUnion::Stage> &stages) {
  for (int i = 0; i <= N_; i++) {
...
void SpatiotemporalUnionHpipmSolver::SetBounds(
    std::vector<SpatiotemporalUnion::Stage> &stages) {
  nbu_[0] = 0;
  ...
  for (int j = 0; j < N_U_; j++) {
    if (stages[0].lower_bounds_u(j) > -SpatiotemporalUnion::INF &&
        stages[0].upper_bounds_u(j) < SpatiotemporalUnion::INF) {
      nbu_[0]++;
  ...
  for (int i = 1; i <= N_; i++) {
    ...
    nbu_[i] = 0;
    for (int j = 0; j < N_U_; j++) {
      if (stages[i].lower_bounds_u(j) > -SpatiotemporalUnion::INF &&
          stages[i].upper_bounds_u(j) < SpatiotemporalUnion::INF) {
        nbu_[i]++;
    ...
    for (int j = 0; j < N_X_; j++) {
      if (stages[i].lower_bounds_x(j) > -SpatiotemporalUnion::INF &&
          stages[i].upper_bounds_x(j) < SpatiotemporalUnion::INF) {
        nbx_[i]++;
...
void SpatiotemporalUnionHpipmSolver::SetPolytopicConstraints(
    std::vector<SpatiotemporalUnion::Stage> &stages) {
  for (int i = 0; i <= N_; i++) {
    ng_[i] = stages[i].ng;
    if (stages[i].ng > 0) {
...
void SpatiotemporalUnionHpipmSolver::SetSoftConstraints(
    std::vector<SpatiotemporalUnion::Stage> &stages) {
  // Note: soft constraints support for polytopic constraints
  for (int i = 0; i <= N_; i++) {
    hpipm_bounds_[i].idx_s.resize(0);
    hpipm_bounds_[i].lower_bounds_s.resize(0);
    hpipm_bounds_[i].upper_bounds_s.resize(0);
    ...
    if (stages[i].ns != 0) {
      for (int j = 0; j < stages[i].ns; j++) {
        if (stages[i].lower_bounds_s[j] >= 1e-6 &&
            stages[i].upper_bounds_s[j] >= 1e-6) {
          nsg_[i]++;

          hpipm_bounds_[i].idx_s.push_back(j + nbx_[i] + nbu_[i]);
          hpipm_bounds_[i].lower_bounds_s.emplace_back(
              stages[i].lower_bounds_s[j]);
          hpipm_bounds_[i].upper_bounds_s.emplace_back(
              stages[i].upper_bounds_s[j]);
        }
      }

      hidxs_[i] = hpipm_bounds_[i].idx_s.data();
      hlls_[i] = hpipm_bounds_[i].lower_bounds_s.data();
      hlus_[i] = hpipm_bounds_[i].upper_bounds_s.data();
...
std::vector<SpatiotemporalUnion::OptVariables>
SpatiotemporalUnionHpipmSolver::SolveMPC(
    const SpatiotemporalUnion::State &x0,
    std::vector<SpatiotemporalUnion::Stage> &stages, int *status) {
  ...
  std::vector<SpatiotemporalUnion::OptVariables> opt_solution = Solve(status);
  opt_solution[0].xk = x0;

  for (const auto &opt_variable : opt_solution) {
    LOG_DEBUG(
        "p_s, v_s, a_s, j_s, jerk_d_s: {:.3f}, {:.3f}, {:.3f}, {:.3f},{:.3f},",
        opt_variable.xk.state_p_s, opt_variable.xk.state_v_s,
        opt_variable.xk.state_a_s, opt_variable.xk.state_j_s,
        opt_variable.uk.control_jerk_d_s);
  }

  for (const auto &opt_variable : opt_solution) {
    LOG_DEBUG(
        "p_l, v_l, a_l, j_l, jerk_d_l: {:.3f}, {:.3f}, {:.3f}, {:.3f},{:.3f},",
        opt_variable.xk.state_p_l, opt_variable.xk.state_v_l,
        opt_variable.xk.state_a_l, opt_variable.xk.state_j_l,
        opt_variable.uk.control_jerk_d_l);
  }
  return opt_solution;
}

std::vector<SpatiotemporalUnion::OptVariables>
SpatiotemporalUnionHpipmSolver::Solve(int *status) {
  // ocp qp dim
  ...
  for (int i = 0; i < N_; i++) {
    ...
    optimal_solution[i].uk = SpatiotemporalUnion::ArrayToControl(u);
  }
  ...
  for (int i = 1; i <= N_; i++) {
    ...
    optimal_solution[i].xk = SpatiotemporalUnion::ArrayToState(x);
  }
...
void SpatiotemporalUnionHpipmSolver::PrintData() {
  for (int k = 0; k <= N_; k++) {
    if (k != N_) {
