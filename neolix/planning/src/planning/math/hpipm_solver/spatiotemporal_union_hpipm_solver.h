...
#include "src/planning/task/optimizers/spatiotemporal_union_optimizer/spatiotemporal_union_model.h"
...
class SpatiotemporalUnionHpipmSolver {
 public:
  SpatiotemporalUnionHpipmSolver(const std::string &solver_name,
                                 const std::size_t n, const std::size_t n_x,
                                 const std::size_t n_u);

  ~SpatiotemporalUnionHpipmSolver() = default;

  std::vector<SpatiotemporalUnion::OptVariables> SolveMPC(
      const SpatiotemporalUnion::State &x0,
      std::vector<SpatiotemporalUnion::Stage> &stages, int *status);

 private:
  void SetDynamics(const SpatiotemporalUnion::State &x0,
                   std::vector<SpatiotemporalUnion::Stage> &stages);

  void SetCost(std::vector<SpatiotemporalUnion::Stage> &stages);

  void SetBounds(std::vector<SpatiotemporalUnion::Stage> &stages);

  void SetPolytopicConstraints(std::vector<SpatiotemporalUnion::Stage> &stages);

  void SetSoftConstraints(std::vector<SpatiotemporalUnion::Stage> &stages);

  std::vector<SpatiotemporalUnion::OptVariables> Solve(int *status);
 ...
 private:
  ...
  std::vector<SpatiotemporalUnion::HpipmBound> hpipm_bounds_;
  Eigen::MatrixXd b0_;
};
