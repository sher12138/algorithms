#include "third_order_spline_path_hpipm_solver.h"
...
ThirdOrderSplinePathHpipmSolver::ThirdOrderSplinePathHpipmSolver(
...
void ThirdOrderSplinePathHpipmSolver::SetDynamics(
    const ThirdOrderSplinePath::State &x0,
    std::vector<ThirdOrderSplinePath::Stage> &stages) {
  b0_ = (stages[0].line_model.A * ThirdOrderSplinePath::StateToVector(x0) +
...
void ThirdOrderSplinePathHpipmSolver::SetCost(
    std::vector<ThirdOrderSplinePath::Stage> &stages) {
  for (int i = 0; i <= N_; i++) {
...
void ThirdOrderSplinePathHpipmSolver::SetBounds(
    std::vector<ThirdOrderSplinePath::Stage> &stages) {
  nbu_[0] = 0;
  ...
  for (int j = 0; j < N_U_; j++) {
    if (stages[0].lower_bounds_u(j) > -ThirdOrderSplinePath::INF &&
        stages[0].upper_bounds_u(j) < ThirdOrderSplinePath::INF) {
      nbu_[0]++;
  ...
  for (int i = 1; i <= N_; i++) {
    ...
    nbu_[i] = 0;
    for (int j = 0; j < N_U_; j++) {
      if (stages[i].lower_bounds_u(j) > -ThirdOrderSplinePath::INF &&
          stages[i].upper_bounds_u(j) < ThirdOrderSplinePath::INF) {
        nbu_[i]++;
    ...
    for (int j = 0; j < N_X_; j++) {
      if (stages[i].lower_bounds_x(j) > -ThirdOrderSplinePath::INF &&
          stages[i].upper_bounds_x(j) < ThirdOrderSplinePath::INF) {
        nbx_[i]++;
...
void ThirdOrderSplinePathHpipmSolver::SetPolytopicConstraints(
    std::vector<ThirdOrderSplinePath::Stage> &stages) {
  for (int i = 0; i <= N_; i++) {
    ng_[i] = stages[i].ng;
    if (stages[i].ng > 0) {
...
void ThirdOrderSplinePathHpipmSolver::SetSoftConstraints(
    std::vector<ThirdOrderSplinePath::Stage> &stages) {
...
std::vector<ThirdOrderSplinePath::OptVariables>
ThirdOrderSplinePathHpipmSolver::SolveMPC(
    const ThirdOrderSplinePath::State &x0,
    std::vector<ThirdOrderSplinePath::Stage> &stages, int *status) {
  ...
  std::vector<ThirdOrderSplinePath::OptVariables> opt_solution = Solve(status);
  opt_solution[0].xk = x0;

  for (const auto &opt_variable : opt_solution) {
    LOG_DEBUG("l0, dl, ddl, dddl: {:.3f}, {:.3f},  {:.3f}, {:.3f}",
              opt_variable.xk.state_l0, opt_variable.xk.state_dl,
              opt_variable.xk.state_ddl, opt_variable.uk.control_dddl);
  }
...
std::vector<ThirdOrderSplinePath::OptVariables>
ThirdOrderSplinePathHpipmSolver::Solve(int *status) {
  // ocp qp dim
  ...
  for (int i = 0; i < N_; i++) {
    ...
    optimal_solution[i].uk = ThirdOrderSplinePath::ArrayToControl(u);
  }
  ...
  for (int i = 1; i <= N_; i++) {
    ...
    optimal_solution[i].xk = ThirdOrderSplinePath::ArrayToState(x);
  }
...
void ThirdOrderSplinePathHpipmSolver::PrintData() {
  for (int k = 0; k <= N_; k++) {
    if (k != N_) {
