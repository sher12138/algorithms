...
class ThirdOrderSplinePathHpipmSolver {
 public:
  ThirdOrderSplinePathHpipmSolver(const std::string &solver_name,
                                  const std::size_t n, const std::size_t n_x,
                                  const std::size_t n_u);

  ~ThirdOrderSplinePathHpipmSolver() = default;

  std::vector<ThirdOrderSplinePath::OptVariables> SolveMPC(
      const ThirdOrderSplinePath::State &x0,
      std::vector<ThirdOrderSplinePath::Stage> &stages, int *status);

 private:
  void SetDynamics(const ThirdOrderSplinePath::State &x0,
                   std::vector<ThirdOrderSplinePath::Stage> &stages);

  void SetCost(std::vector<ThirdOrderSplinePath::Stage> &stages);

  void SetBounds(std::vector<ThirdOrderSplinePath::Stage> &stages);

  void SetPolytopicConstraints(
      std::vector<ThirdOrderSplinePath::Stage> &stages);

  void SetSoftConstraints(std::vector<ThirdOrderSplinePath::Stage> &stages);

  std::vector<ThirdOrderSplinePath::OptVariables> Solve(int *status);
 ...
 private:
  ...
  std::vector<ThirdOrderSplinePath::HpipmBound> hpipm_bounds_;
  Eigen::MatrixXd b0_;
};
