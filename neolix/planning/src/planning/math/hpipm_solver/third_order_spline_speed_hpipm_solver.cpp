#include "third_order_spline_speed_hpipm_solver.h"
...
ThirdOrderSplineSpeedHpipmSolver::ThirdOrderSplineSpeedHpipmSolver(
...
void ThirdOrderSplineSpeedHpipmSolver::SetPolytopicConstraints(
    std::vector<ThirdOrderSplineSpeed::Stage> &stages) {
  for (int i = 0; i <= N_; i++) {
    ng_[i] = stages[i].ng;
    if (stages[i].ng > 0) {
...
std::vector<ThirdOrderSplineSpeed::OptVariables>
ThirdOrderSplineSpeedHpipmSolver::SolveMPC(
    const ThirdOrderSplineSpeed::State &x0,
  ...
  std::vector<ThirdOrderSplineSpeed::OptVariables> opt_solution = Solve(status);
...
std::vector<ThirdOrderSplineSpeed::OptVariables>
ThirdOrderSplineSpeedHpipmSolver::Solve(int *status) {
  // ocp qp dim
  ...
  for (int i = 0; i < N_; i++) {
    ...
    optimal_solution[i].uk = ThirdOrderSplineSpeed::ArrayToControl(u);
  }
  ...
  for (int i = 1; i <= N_; i++) {
    ...
    optimal_solution[i].xk = ThirdOrderSplineSpeed::ArrayToState(x);
  }
...
void ThirdOrderSplineSpeedHpipmSolver::PrintData() {
  for (int k = 0; k <= N_; k++) {
    if (k != N_) {
