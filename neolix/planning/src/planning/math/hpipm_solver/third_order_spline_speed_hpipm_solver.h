...
#include "src/planning/task/optimizers/third_order_spline_speed_optimizer/third_order_spline_speed_model.h"
...
class ThirdOrderSplineSpeedHpipmSolver {
 public:
  ThirdOrderSplineSpeedHpipmSolver(const std::string &solver_name,
                                   const std::size_t n, const std::size_t n_x,
                                   const std::size_t n_u);

  ~ThirdOrderSplineSpeedHpipmSolver() = default;

  std::vector<ThirdOrderSplineSpeed::OptVariables> SolveMPC(
      const ThirdOrderSplineSpeed::State &x0,
      std::vector<ThirdOrderSplineSpeed::Stage> &stages, int *status);

 private:
  void SetDynamics(const ThirdOrderSplineSpeed::State &x0,
                   std::vector<ThirdOrderSplineSpeed::Stage> &stages);

  void SetCost(std::vector<ThirdOrderSplineSpeed::Stage> &stages);

  void SetBounds(std::vector<ThirdOrderSplineSpeed::Stage> &stages);

  void SetPolytopicConstraints(
      std::vector<ThirdOrderSplineSpeed::Stage> &stages);

  void SetSoftConstraints(std::vector<ThirdOrderSplineSpeed::Stage> &stages);

  std::vector<ThirdOrderSplineSpeed::OptVariables> Solve(int *status);
