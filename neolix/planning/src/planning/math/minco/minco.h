...
class Piece {
 ...
 public:
  ...
  inline double getCurv(const double &t) const {
    ...
    return dsigma.norm() < 1e-6
               ? 0.0
               : (singul * (dsigma(0) * ddsigma(1) - dsigma(1) * ddsigma(0)) /
  ...
  inline double getAcc(const double &t) const {
    ...
    return dsigma.norm() < 1e-6
               ? 0.0
               : (singul * (dsigma(0) * ddsigma(0) + dsigma(1) * ddsigma(1)) /
                  dsigma.norm());
  }
...
class Trajectory {
 ...
 public:
  ...
  inline Eigen::Vector2d getJuncPos(int juncIdx) const {
    return juncIdx != getPieceNum()
               ? pieces[juncIdx].getCoeffMat().col(5)
               : pieces[juncIdx - 1].getPos(pieces[juncIdx - 1].getDuration());
  }

  inline Eigen::Vector2d getJuncdSigma(int juncIdx) const {
    return juncIdx != getPieceNum() ? pieces[juncIdx].getCoeffMat().col(4)
                                    : pieces[juncIdx - 1].getdSigma(
                                          pieces[juncIdx - 1].getDuration());
  }

  inline Eigen::Vector2d getJuncddSigma(int juncIdx) const {
    return juncIdx != getPieceNum() ? pieces[juncIdx].getCoeffMat().col(3) * 2.0
                                    : pieces[juncIdx - 1].getddSigma(
                                          pieces[juncIdx - 1].getDuration());
  }
...
class MinJerkOpt {
 ...
 private:
  ...
  inline void addPropCtoP(const Eigen::MatrixXd &adjGdC,
    ...
    // gdHead << adjGdC.topRows(3).transpose();
    // gdTail << adjGdC.bottomRows(3).transpose();
 ...
 public:
  ...
  inline void evalTrajCostGrad(const Eigen::VectorXi &cons,
    ...
    addTimeIntPenalty(cons, idxHs, ci, cost, gdT, gdC);
  ...
  inline Eigen::MatrixXd &getHead() { return headPVA; }

  inline Eigen::MatrixXd &getTail() { return tailPVA; }

  inline Eigen::MatrixXd get_gdHead() { return gdHead; }

  inline Eigen::MatrixXd get_gdTail() { return gdTail; }
};
