...
GridSearch::GridSearch(const ParkingHybridAStarConfig& config) {
  xy_grid_resolution_ = config.grid_a_star_xy_resolution_;
  node_radius_ = config.node_radius_;
  step_length_ = config.grid_sample_step_length_;
}
...
bool GridSearch::GenerateAStarPath(
  ...
  if (final_node_ == nullptr) {
    LOG_ERROR("Grid A searching return null ptr(open_set ran out)");
    return false;
  }
  LoadGridAStarResult(result);
  NL_INFO << "explored node num is " << explored_node_num;
  return true;
}
...
bool GridSearch::GenerateDpMap(
  ...
  LOG_INFO("dp_map explored node num is {}", explored_node_num);
  return true;
}
...
std::vector<std::shared_ptr<Node2d>> GridSearch::GenerateNextNodes(
  ...
  // set step length
  double diagonal_distance = std::sqrt(step_length_ * step_length_ * 2);
  std::vector<std::shared_ptr<Node2d>> next_nodes;
  std::shared_ptr<Node2d> up = std::make_shared<Node2d>(
      current_node_x, current_node_y + step_length_, XYbounds_);
  up->SetPathCost(current_node_path_cost + step_length_);
  std::shared_ptr<Node2d> up_right = std::make_shared<Node2d>(
      current_node_x + step_length_, current_node_y + step_length_, XYbounds_);
  up_right->SetPathCost(current_node_path_cost + diagonal_distance);
  std::shared_ptr<Node2d> right = std::make_shared<Node2d>(
      current_node_x + step_length_, current_node_y, XYbounds_);
  right->SetPathCost(current_node_path_cost + step_length_);
  ...
  std::shared_ptr<Node2d> down = std::make_shared<Node2d>(
      current_node_x, current_node_y - step_length_, XYbounds_);
  down->SetPathCost(current_node_path_cost + step_length_);
  ...
  std::shared_ptr<Node2d> left = std::make_shared<Node2d>(
      current_node_x - step_length_, current_node_y, XYbounds_);
  left->SetPathCost(current_node_path_cost + step_length_);
...
bool GridSearch::CheckConstraints(std::shared_ptr<Node2d> node) {
  ...
  for (const auto& obstacle_linesegments : obstacles_linesegments_vec_) {
    for (const Segment2d& linesegment : obstacle_linesegments) {
      if (linesegment.distance_to({node->GetGridX(), node->GetGridY()}) <
          node_radius_) {
        return false;
      }
    }
