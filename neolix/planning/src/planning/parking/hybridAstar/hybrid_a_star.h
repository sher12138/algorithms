...
struct HybridAStartResult {
  ...
  std::vector<int> direction;  // 0:forward;1:backward;
  std::vector<int> segment_index;
};

class HybridAStar {
 public:
  explicit HybridAStar(const ParkingHybridAStarConfig config,
 ...
 private:
  ...
  //**generate path only
  bool RetrieveResult(HybridAStartResult* result);

  bool GetdirectionSegment(HybridAStartResult* result);
  ...
  bool TrajectoryPartitionInner(
      const HybridAStartResult& result,
      std::vector<HybridAStartResult>* partitioned_result);

  bool GetTemporalSpeedProfile(HybridAStartResult* result);
