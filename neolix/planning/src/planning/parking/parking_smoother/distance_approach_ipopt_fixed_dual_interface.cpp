...
bool DistanceApproachIPOPTFixedDualInterface::GetNlpInfo(
...
bool DistanceApproachIPOPTFixedDualInterface::get_starting_point(
  ...
  if (distance_approach_config_.enable_check_initial_state_) {
    int kM = m;
    double g[kM];
    EvalG(n, x, true, m, g);
    CheckG(n, x, m, g);
...
void DistanceApproachIPOPTFixedDualInterface::get_optimization_results(
  ...
  LOG_INFO("get_optimization_results");
...
void DistanceApproachIPOPTFixedDualInterface::eval_constraints(int n,
  ...
  LOG_INFO("constraint_index after adding variable box constraints updated: {}",
           constraint_index);
  return;
}
...
bool DistanceApproachIPOPTFixedDualInterface::check_g(int n, const double* x,
  ...
  if (m9 != num_of_constraints_) {
    LOG_ERROR("m9 != num_of_constraints_");
    return false;
  }
