...
DistanceApproachIPOPTInterface::DistanceApproachIPOPTInterface(
  ...
  enable_jacobian_ad_ = distance_approach_config_.enable_jacobian_ad_;
}

bool DistanceApproachIPOPTInterface::GetNlpInfo(int& n, int& m, int& nnz_jac_g,
                                                int& nnz_h_lag,
...
bool DistanceApproachIPOPTInterface::GetBoundsInfo(int n, double* x_l,
                                                   double* x_u, int m,
                                                   double* g_l, double* g_u) {
  LOG_INFO("GetBoundsInfo");
  if (XYbounds_.size() != 4) {
  ...
  LOG_INFO("sample time fix time status is : {}", use_fix_time_);
  ...
  LOG_INFO("constraint_index after adding time constraints: {}",
           constraint_index);
...
bool DistanceApproachIPOPTInterface::GetStartingPoint(int n, bool init_x,
...
bool DistanceApproachIPOPTInterface::EvalF(int n, const double* x, bool new_x,
                                           double& obj_value) {
  EvalObj(n, x, &obj_value);
  return true;
}

bool DistanceApproachIPOPTInterface::EvalGradF(int n, const double* x,
                                               bool new_x, double* grad_f) {
  gradient(tag_f, n, x, grad_f);
  return true;
}

bool DistanceApproachIPOPTInterface::EvalG(int n, const double* x, bool new_x,
                                           int m, double* g) {
  EvalConstraints(n, x, m, g);
  // if (enable_constraint_check_) CheckG(n, x, m, g);
  return true;
}

bool DistanceApproachIPOPTInterface::EvalJacG(int n, const double* x,
...
bool DistanceApproachIPOPTInterface::EvalJacGSer(int n, const double* x,
                                                 bool new_x, int m,
                                                 int nele_jac, int* iRow,
                                                 int* jCol, double* values) {
  LOG_INFO("EvalJacG");
  if (n != num_of_variables_) {
    LOG_ERROR("No. of variables [{}] wrong in EvalJacG. n : {} ",
              num_of_variables_, n);
    return false;
  }
  if (m != num_of_constraints_) {
    LOG_ERROR("No. of constraints [{}] wrong in EvalJacG. n : {} ",
  ...
  if (values == nullptr) {
    ...
    if (nz_index != static_cast<int>(nele_jac)) {
      LOG_ERROR("nz_index [{}]!= nele_jac [{}]", nz_index, nele_jac);
      return false;
    }
    if (constraint_index != static_cast<int>(m)) {
    ...
    // TODO: initially implemented to be debug friendly, later iterate
    ...
    LOG_INFO(
        "After fulfilled time constraints derivative, nz_index : {}, nele_jac "
        ": {}",
        nz_index, nele_jac);
  ...
  LOG_INFO("EvalJacG done");
  return true;
}  // NOLINT

bool DistanceApproachIPOPTInterface::EvalH(int n, const double* x, bool new_x,
...
void DistanceApproachIPOPTInterface::FinalizeSolution(
...
void DistanceApproachIPOPTInterface::GetOptimizationResults(
...
//***************    start ADOL-C part ***********************************
template <class T>
void DistanceApproachIPOPTInterface::EvalObj(int n, const T* x, T* obj_value) {
  // Objective is :
  // min control inputs
  // min input rate
  ...
  // TODO: Initial implementation towards earlier understanding and debug
...
template <class T>
void DistanceApproachIPOPTInterface::EvalConstraints(int n, const T* x, int m,
                                                     T* g) {
  // state start index
  int state_index = state_start_index_;
  ...
  for (int i = 0; i < horizon_; ++i) {
    ...
    // TODO: evaluate performance of different models
    // g[constraint_index] =
    //     x[state_index + 4] -
    //     (x[state_index] +
  ...
  LOG_INFO(
      "constraint_index after adding time constraints "
      "updated: {}",
      constraint_index);
...
bool DistanceApproachIPOPTInterface::CheckG(int n, const double* x, int m,
                                            const double* g) {
  int kN = n;
  int kM = m;
  double x_u_tmp[kN];
  ...
  if (m11 != num_of_constraints_) {
    LOG_ERROR("m11 [{}] != num_of_constraints_ [{}]", m11, num_of_constraints_);
    return false;
  }
...
void DistanceApproachIPOPTInterface::GenerateTapes(int n, int m, int* nnz_jac_g,
                                                   int* nnz_h_lag) {
