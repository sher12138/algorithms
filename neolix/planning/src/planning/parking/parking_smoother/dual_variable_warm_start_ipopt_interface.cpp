...
DualVariableWarmStartIPOPTInterface::DualVariableWarmStartIPOPTInterface(
  ...
  weight_d_ = dualvariable_config.weight_d_;
}

bool DualVariableWarmStartIPOPTInterface::GetNlpInfo(
...
bool DualVariableWarmStartIPOPTInterface::GetStartingPoint(
  ...
  LOG_INFO("d_start_index_ : {}", d_start_index_);
...
bool DualVariableWarmStartIPOPTInterface::GetBoundsInfo(int n, double* x_l,
                                                        double* x_u, int m,
                                                        double* g_l,
                                                        double* g_u) {
  ...
  LOG_INFO("variable_index after adding d : {}", variable_index);
...
bool DualVariableWarmStartIPOPTInterface::EvalF(int n, const double* x,
                                                bool new_x, double& obj_value) {
  EvalObj(n, x, &obj_value);
  return true;
}

bool DualVariableWarmStartIPOPTInterface::EvalGradF(int n, const double* x,
                                                    bool new_x,
                                                    double* grad_f) {
...
bool DualVariableWarmStartIPOPTInterface::EvalG(int n, const double* x,
                                                bool new_x, int m, double* g) {
  EvalConstraints(n, x, m, g);
  return true;
}

bool DualVariableWarmStartIPOPTInterface::EvalJacG(int n, const double* x,
                                                   bool new_x, int m,
...
bool DualVariableWarmStartIPOPTInterface::EvalH(int n, const double* x,
...
void DualVariableWarmStartIPOPTInterface::FinalizeSolution(
...
void DualVariableWarmStartIPOPTInterface::GetOptimizationResults(
...
template <class T>
bool DualVariableWarmStartIPOPTInterface::EvalObj(int n, const T* x,
                                                  T* obj_value) {
  *obj_value = 0.0;
  int d_index = d_start_index_;
  for (int i = 0; i < horizon_ + 1; ++i) {
...
template <class T>
bool DualVariableWarmStartIPOPTInterface::EvalConstraints(int n, const T* x,
                                                          int m, T* g) {
  // state start index
  ...
  for (int i = 0; i < horizon_ + 1; ++i) {
    ...
    for (int j = 0; j < obstacles_num_; ++j) {
      ...
      // TODO: Need to revise according to dual modeling
      T tmp3 = 0.0;
      for (int k = 0; k < 4; ++k) {
        tmp3 += g_[k] * x[n_index + k];
      }
...
void DualVariableWarmStartIPOPTInterface::GenerateTapes(int n, int m,
                                                        int* nnz_h_lag) {
