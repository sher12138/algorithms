...
DualVariableWarmStartOSQPInterface::DualVariableWarmStartOSQPInterface(
  ...
  min_safety_distance_ = dualvariable_config_.min_safety_distance_;
  check_mode_ = dualvariable_config_.osqp_config_.debug_osqp_;
}
...
void printMatrix(const int r, const int c, const std::vector<c_float>& P_data,
  ...
  LOG_INFO("row number: {}, col number: {}", r, c);
  return;
}
...
bool DualVariableWarmStartOSQPInterface::optimize() {
  ...
  if (check_mode_) {
    LOG_INFO("print P_data in whole: ");
  ...
  if (check_mode_) {
    LOG_INFO("print A_data in whole: ");
  ...
  settings->alpha =
      dualvariable_config_.osqp_config_.alpha_;  // Change alpha parameter
  settings->eps_abs = dualvariable_config_.osqp_config_.eps_abs_;
  settings->eps_rel = dualvariable_config_.osqp_config_.eps_rel_;
  settings->max_iter = dualvariable_config_.osqp_config_.max_iter_;
  settings->polish = dualvariable_config_.osqp_config_.polish_;
  settings->verbose = dualvariable_config_.osqp_config_.verbose_;
...
void DualVariableWarmStartOSQPInterface::CheckSolution(
  ...
  for (int idx = r3_index; idx < r4_index; ++idx) {
    if (g(idx, 0) < 0) {
      LOG_ERROR("lambda box constraint fails, constraint_index: {}, g: {}", idx,
                g(idx, 0));
    }
  }
  ...
  for (int idx = r4_index; idx < num_of_constraints_; ++idx) {
    if (g(idx, 0) < 0) {
      LOG_ERROR("miu box constraint fails, constraint_index: {}, g: {}", idx,
                g(idx, 0));
    }
  }
  return;
...
void DualVariableWarmStartOSQPInterface::AssembleP(
  ...
  for (int j = 0; j < obstacles_num_; ++j) {
    ...
    if (current_edges_num != Aj.cols()) {
      LOG_ERROR("current_edges_num [{}] != Aj.cols() [{}]", current_edges_num,
                Aj.cols());
      return;
    }
    if (current_edges_num != Aj.rows()) {
      LOG_ERROR("current_edges_num [{}] != Aj.rows() [{}]", current_edges_num,
                Aj.rows());
      return;
    }
  ...
  if (P_indptr->size() != lambda_horizon_) {
    LOG_ERROR("P_indptr->size() [{}] != lambda_horizon_ [{}]",
              P_indptr->size(), lambda_horizon_);
    return;
  }
  ...
  if (P_data->size() != P_indices->size()) {
    LOG_ERROR("P_data->size() [{}] != P_indices->size() [{}]", P_data->size(),
              P_indices->size());
    return;
  }
  if (P_indptr->size() != num_of_variables_ + 1) {
    LOG_ERROR("P_indptr->size() [{}] != num_of_variables_+1 [{}]",
              P_indptr->size(), num_of_variables_ + 1);
    return;
  }
  return;
}

void DualVariableWarmStartOSQPInterface::AssembleConstraint(
  ...
  if (A_data->size() != A_indices->size()) {
    LOG_ERROR("A_data->size() [{}] != A_indices->size() [{}]", A_data->size(),
              A_indices->size());
    return;
  }
  if (A_indptr->size() != num_of_variables_ + 1) {
    LOG_ERROR("A_indptr->size() [{}] != num_of_variables_ + 1 [{}]",
              A_indptr->size(), num_of_variables_ + 1);
    return;
  }
  return;
}

void DualVariableWarmStartOSQPInterface::GetOptimizationResults(
