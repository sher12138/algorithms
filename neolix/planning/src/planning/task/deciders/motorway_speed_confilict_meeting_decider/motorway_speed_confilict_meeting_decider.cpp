#include "motorway_speed_confilict_meeting_decider.h"
...
MotorwaySpeedConfilictMeetingDecider::MotorwaySpeedConfilictMeetingDecider() {
  name_ = "MotorwaySpeedConfilictMeetingDecider";
}
...
ErrorCode MotorwaySpeedConfilictMeetingDecider::Execute(TaskInfo& task_info) {
...
void MotorwaySpeedConfilictMeetingDecider::SaveTaskResults(
    TaskInfo& task_info) {
...
bool MotorwaySpeedConfilictMeetingDecider::Init(TaskInfo& task_info) {
  if (!update_speed_limit_) {
    last_limited_speed_ =
...
bool MotorwaySpeedConfilictMeetingDecider::Process(TaskInfo& task_info) {
  if (lane_id_s_.empty()) {
  ...
  accelerate_ = std::max(-3.0, accelerate_ < 0.0 ? accelerate_ : 0.0);
...
bool MotorwaySpeedConfilictMeetingDecider::JudgeHasCollisionWithEGO(
