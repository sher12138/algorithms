...
ConflictDataBasePrediction::ConflictDataBasePrediction()
    : ConflictDataInterface("ConflictDataBaseOnPrediction") {}

std::vector<ConnectionConflictInfo>
ConflictDataBasePrediction::ComputeConflictMergeInData(TaskInfo& task_info) {
...
std::vector<ConnectionConflictInfo>
ConflictDataBasePrediction::ComputeConflictMeetingData(TaskInfo& task_info) {
...
void ConflictDataBasePrediction::CalcMeeting(const PredictionCurves& ego,
...
void ConflictDataBasePrediction::CalcConflictZone(
    const std::vector<Vec2d>& ego, const std::vector<Vec2d>& agent,
    std::array<math::AD2, 4>& res, math::AD2& ego_rd, math::AD2& agent_rd) {
...
std::vector<ConnectionConflictInfo>
ConflictDataBasePrediction::ComputeConflictCustomData(TaskInfo& task_info) {
  std::vector<ConnectionConflictInfo> ans{};
  if (interactive_agent_ids_.empty()) {
    LOG_INFO("No Dynamic Obs!");
    return ans;
...
void ConflictDataBasePrediction::CalcCircle(const double width,
                                            const double length, const size_t n,
                                            double& r, double& d) {
...
void ConflictDataBasePrediction::CalcCirclePoint(
...
bool ConflictDataBasePrediction::CalcAgentCurves(
  ...
  const auto& pred_trajs = obs->prediction_trajectories();
  if (enable_prediction && pred_trajs.empty()) {
    LOG_ERROR("pred_trajs empty.");
    return false;
  }
  const auto& pred_traj =
      enable_prediction ? pred_trajs.front() : obs->uniform_trajectory();
...
bool ConflictDataBasePrediction::CalcEgoCurves(TaskInfo& task_info,
                                               PredictionCurves& ego_curves,
                                               std::vector<Vec2d>& ego_traj) {
