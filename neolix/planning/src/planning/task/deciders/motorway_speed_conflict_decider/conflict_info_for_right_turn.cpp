...
ConflictInfoForRightTurn::ConflictInfoForRightTurn()
    : ConflictInfoInterface("ConflictInfoForRightTurn") {}

std::vector<ConnectionConflictInfo>
ConflictInfoForRightTurn::ComputeConflictInfo(TaskInfo& task_info) {
  ...
  const auto& curr_stage = DataCenter::Instance()
                               ...
                               .stage;
  if ((curr_stage != MotorwayIntersectionStageState::TURN_RIGHT)) {
    return {};
  }
  /// obstacle filter
  ...
  for (const auto& iter : dynamic_obstacle) {
    if (iter->type() == Obstacle::ObstacleType::PEDESTRIAN) {
      LOG_INFO("right turn and meeting: obs's type is pedestrian, id is {}!",
               iter->id());
      continue;
    }
  ...
  cdbp.set_interactive_agent_ids(
      meeting_agent_ids_,
      neodrive::planning::MotorwayInteractiveType::RIGHT_MEETING);
  ...
  for (const auto& iter : dynamic_obstacle) {
    if (iter->type() == Obstacle::ObstacleType::PEDESTRIAN) {
      LOG_INFO("right turn and merge: obs's type is pedestrian, id is {}!",
               iter->id());
      continue;
    }
  ...
  cdbp.set_interactive_agent_ids(
      merge_in_agent_ids_,
      neodrive::planning::MotorwayInteractiveType::RIGHT_MERGE);
