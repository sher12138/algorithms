...
MotorwaySpeedConflictDecider::MotorwaySpeedConflictDecider() {
  name_ = "MotorwaySpeedConflictDecider";
}

MotorwaySpeedConflictDecider::~MotorwaySpeedConflictDecider() { Reset(); }

bool MotorwaySpeedConflictDecider::Init(TaskInfo& task_info) {
  ...
  auto conflict_info_left =
      ConflictInfoForLeftTurn{}.ComputeConflictInfo(task_info);
  if (SteerInitialFilter(conflict_info_left)) {
    LOG_INFO("Steering start filtering: left!");
  }
  auto conflict_info_right =
      ConflictInfoForRightTurn{}.ComputeConflictInfo(task_info);
  if (SteerInitialFilter(conflict_info_right)) {
    LOG_INFO("Steering start filtering: right!");
  }
  auto conflict_info_straight =
      ConflictInfoForStraight{}.ComputeConflictInfo(task_info);
  LOG_INFO("Straight agents size is {}", conflict_info_straight.size());
  auto conflict_info_merge_in =
      ConflictInfoForMergeIn{}.ComputeConflictInfo(task_info);
  auto conflict_info_for_diverging =
      ConflictInfoForDiverging::Instance().ComputeConflictInfo(task_info);

  std::vector<ConnectionConflictInfo> all_conflict_infos{};
  all_conflict_infos.insert(all_conflict_infos.end(),
                            conflict_info_left.begin(),
                            conflict_info_left.end());
  all_conflict_infos.insert(all_conflict_infos.end(),
                            conflict_info_right.begin(),
                            conflict_info_right.end());
  all_conflict_infos.insert(all_conflict_infos.end(),
                            conflict_info_straight.begin(),
                            conflict_info_straight.end());
  all_conflict_infos.insert(all_conflict_infos.end(),
                            conflict_info_merge_in.begin(),
                            conflict_info_merge_in.end());
  all_conflict_infos.insert(all_conflict_infos.end(),
                            conflict_info_for_diverging.begin(),
                            conflict_info_for_diverging.end());
...
ErrorCode MotorwaySpeedConflictDecider::Execute(TaskInfo& task_info) {
  ...
  if (risk_prob_ >= 0.0) {
    ...
    if (!is_redecision(conflict_area_rush_bound_[2], special_t_)) {
      SubmitSpeedDecision(task_info);
...
bool MotorwaySpeedConflictDecider::CalPostProcessDataForSkip(
    TaskInfo& task_info) {
...
void MotorwaySpeedConflictDecider::SubmitSpeedDecision(TaskInfo& task_info) {
  task_info.current_frame()
      ->mutable_outside_planner_data()
      ->motorway_speed_context.goal_decision.emplace_back(
          std::move(goal_decision_));
...
bool MotorwaySpeedConflictDecider::CalPostProcessData(
...
MotorwaySpeedConflictDecider::CombineConflictInfos(
...
bool MotorwaySpeedConflictDecider::set_speed_limit(
...
bool MotorwaySpeedConflictDecider::set_exact_limit() {
...
bool MotorwaySpeedConflictDecider::set_rush_limit(TaskInfo& task_info) {
...
bool MotorwaySpeedConflictDecider::set_yield_limit(TaskInfo& task_info) {
  ...
  if (!upstream_speed_limit_.empty() && !upstream_s_limit_.empty() &&
      upstream_speed_limit_.size() == upstream_s_limit_.size()) {
    // upstream_speed_limit_.erase(upstream_speed_limit_.begin());
    // upstream_s_limit_.erase(upstream_s_limit_.begin());
    // upstream_a_limit_.erase(upstream_a_limit_.begin());
    return true;
  }
  LOG_ERROR("Upstream Limit is Empty!");
  return false;
...
bool MotorwaySpeedConflictDecider::SplitFRAreaBound() {
...
bool MotorwaySpeedConflictDecider::CalInteractiveAgent(TaskInfo& task_info) {
  uint64_t sig_fr_size = sigment_fr_bounds_.size();
  uint64_t connect_areas_size = process_data_.size();
  std::vector<ConnectionConflictInfo> pending_interactive_cases =
      process_data_[connect_areas_size - sig_fr_size];
...
bool MotorwaySpeedConflictDecider::CalInputData(TaskInfo& task_info) {
  auto outside_data_ptr =
      task_info.current_frame()->mutable_outside_planner_data();
...
bool MotorwaySpeedConflictDecider::CalRushProbByGameTheory(
...
bool MotorwaySpeedConflictDecider::FilterFrontDynamicObs(TaskInfo& task_info,
                                                         const int obs_id) {
  ...
  for (const auto& obs_decision : multi_cipv_dynamic_obstacles_decision) {
    if (obs_decision.obstacle.id() == obs_id) {
      return true;
    }
  }
  return false;
...
bool MotorwaySpeedConflictDecider::is_redecision(
    const double rear_bound, const double constraints_time) {
...
bool MotorwaySpeedConflictDecider::SteerInitialFilter(
