#include "motorway_speed_crosswalk_pedestrian_protect_decider.h"
...
MotorwaySpeedCrosswalkPedestrainProtectDecider::
    MotorwaySpeedCrosswalkPedestrainProtectDecider() {
  name_ = "MotorwaySpeedCrosswalkPedestrainProtectDecider";
}

void MotorwaySpeedCrosswalkPedestrainProtectDecider::GetPedestrianCheckArea(
    TaskInfo& task_info) {
  if (if_next_to_crosswalk_) {
    ...
    for (const auto& crosswalk_info : lane_cross_walk_seq) {
...
void MotorwaySpeedCrosswalkPedestrainProtectDecider::CrosswalkPoly2Boundary(
...
ErrorCode MotorwaySpeedCrosswalkPedestrainProtectDecider::Execute(
    TaskInfo& task_info) {
...
void MotorwaySpeedCrosswalkPedestrainProtectDecider::SetSpeedLimit(
    double speed_limit, TaskInfo& task_info, bool if_hard) {
...
void MotorwaySpeedCrosswalkPedestrainProtectDecider::Reset() {
...
bool MotorwaySpeedCrosswalkPedestrainProtectDecider::Init(TaskInfo& task_info) {
...
bool MotorwaySpeedCrosswalkPedestrainProtectDecider::IfInCrosswalkCheckArea(
    TaskInfo& task_info) {
...
bool MotorwaySpeedCrosswalkPedestrainProtectDecider::Process(
    TaskInfo& task_info) {
...
bool MotorwaySpeedCrosswalkPedestrainProtectDecider::ProcessProtect(
    TaskInfo& task_info) {
...
void MotorwaySpeedCrosswalkPedestrainProtectDecider::
    CheckStaticPedestrianInCheckArea(TaskInfo& task_info) {
  ...
  for (std::size_t i = 0; i < static_obs_vec.size(); ++i) {
    ...
    auto& adc_boundaries =
        outside_planner_data_ptr->motorway_speed_obstacle_context
            .adc_sl_boundaries;
...
void MotorwaySpeedCrosswalkPedestrainProtectDecider::
    CheckDynamicPedestrianInCheckArea(TaskInfo& task_info) {
  const auto& inside_data = task_info.current_frame()->inside_planner_data();
...
void MotorwaySpeedCrosswalkPedestrainProtectDecider::SaveTaskResults(
    TaskInfo& task_info) {}
