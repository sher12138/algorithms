...
MotorwaySpeedLimitReverseLaneDetour::MotorwaySpeedLimitReverseLaneDetour()
    : MotorwaySpeedLimitInterface("motorway_speed_limit_reverse_lane_detour") {}

void MotorwaySpeedLimitReverseLaneDetour::ComputeSpeedLimit(
    TaskInfo &task_info) {
  ...
  const auto &conf = config::PlanningConfig::Instance()
                         ->plan_config()
                         .reverse_lane_detour.speed_limit_motorway;
