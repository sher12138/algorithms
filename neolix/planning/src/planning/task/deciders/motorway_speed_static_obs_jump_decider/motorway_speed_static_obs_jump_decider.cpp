#include "motorway_speed_static_obs_jump_decider.h"
...
MotorwaySpeedStaticObsJumpDecider::MotorwaySpeedStaticObsJumpDecider() {
  name_ = "MotorwaySpeedStaticObsJumpDecider";
}
...
ErrorCode MotorwaySpeedStaticObsJumpDecider::Execute(TaskInfo& task_info) {
...
void MotorwaySpeedStaticObsJumpDecider::SaveTaskResults(TaskInfo& task_info) {
...
void MotorwaySpeedStaticObsJumpDecider::Reset(){};

bool MotorwaySpeedStaticObsJumpDecider::Init(TaskInfo& task_info) {
...
bool MotorwaySpeedStaticObsJumpDecider::Process(TaskInfo& task_info) {
  ComputeCollideInfo(task_info);

  auto& ignore_static_obs_id =
      task_info.current_frame()
          ->mutable_outside_planner_data()
          ->motorway_speed_obstacle_context.ignore_static_obs_id;
  for (const auto& [id, collide_info] : collide_infos_) {
    if (collide_info.path_lat_dis >
      ...
      // Ignore
      ignore_static_obs_id.insert(id);
    }
  }
...
bool MotorwaySpeedStaticObsJumpDecider::DataCheck(TaskInfo& task_info) {
...
void MotorwaySpeedStaticObsJumpDecider::ComputeCollideInfo(
    TaskInfo& task_info) {
  const auto& multi_cipv_static_obstacles_decision =
      task_info.current_frame()
          ->outside_planner_data()
          .motorway_speed_obstacle_context.multi_cipv_static_obstacles_decision;
  if (multi_cipv_static_obstacles_decision.empty()) return;
  ...
  for (const auto& obs_info : multi_cipv_static_obstacles_decision) {
...
double MotorwaySpeedStaticObsJumpDecider::ComputeSpeedLimit(
...
double MotorwaySpeedStaticObsJumpDecider::ComputeEndSpeedLimit(
    int obs_id, const StaticObsDisInfo& collision_info) {
...
void MotorwaySpeedStaticObsJumpDecider::ClearObsHistoryInfo() {
...
void MotorwaySpeedStaticObsJumpDecider::UpdataObsHistoryInfo(
    TaskInfo& task_info) {
