...
#include "motorway_speed_unprotected_turn_decider.h"
...
MotorwaySpeedUnprotectedTurnDecider::MotorwaySpeedUnprotectedTurnDecider() {
  name_ = "MotorwaySpeedUnprotectedTurnDecider";
}

MotorwaySpeedUnprotectedTurnDecider::~MotorwaySpeedUnprotectedTurnDecider() {}

ErrorCode MotorwaySpeedUnprotectedTurnDecider::Execute(TaskInfo& task_info) {
  LOG_INFO(">>>> start execute MotorwaySpeedUnprotectedTurnDecider");
  if (!Init(task_info)) {
    LOG_INFO("MotorwaySpeedUnprotectedTurnDecider init failed.");
    return ErrorCode::PLANNING_ERROR_FAILED;
  }
  if (!Process(task_info)) {
    LOG_INFO("MotorwaySpeedUnprotectedTurnDecider process failed.");
    return ErrorCode::PLANNING_ERROR_FAILED;
  }
  return ErrorCode::PLANNING_OK;
}

void MotorwaySpeedUnprotectedTurnDecider::SaveTaskResults(TaskInfo& task_info) {
...
bool MotorwaySpeedUnprotectedTurnDecider::Init(TaskInfo& task_info) {
...
bool MotorwaySpeedUnprotectedTurnDecider::Process(TaskInfo& task_info) {
  ...
  const auto& curr_stage = DataCenter::Instance()
                               ...
                               .stage;
  if (curr_stage == MotorwayIntersectionStageState::PROTECTED_TURN_LEFT) {
    LOG_INFO("ego in protected turn left, don't handle.");
    return true;
  }
...
bool MotorwaySpeedUnprotectedTurnDecider::ObsIsInEgoTurnDirection(
    const Obstacle& obs) {
...
bool MotorwaySpeedUnprotectedTurnDecider::GenerateDecision(
  ...
  double collision_time =
      std::sqrt(std::pow(collision_x - obs.center().x(), 2) +
                std::pow(collision_y - obs.center().y(), 2)) /
      obs.speed();
...
bool MotorwaySpeedUnprotectedTurnDecider::VehicleCoordinationBehind(
    const Obstacle& obs) {
  ...
  return true ? obs_local_x < 0. : false;
}

void MotorwaySpeedUnprotectedTurnDecider::ReMapBaseRefLine(
    const ReferenceLinePtr& ref_line) {
