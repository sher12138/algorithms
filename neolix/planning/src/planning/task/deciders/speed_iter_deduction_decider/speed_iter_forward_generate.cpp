...
namespace neodrive {
namespace planning {
HybirdPid::HybirdPid(const IterDeductionEgoState& ego_state,
  ...
  if (DataCenter::Instance()->master_info().curr_scenario() ==
    ...
    desire_max_a_ =
        config::PlanningConfig::Instance()->plan_config().parking.max_jerk;
  }
...
DeductionEgoStateSequence HybirdPid::CalPidProcess() {
...
HybirdIDM::HybirdIDM(const IterDeductionEgoState& ego_state_idm,
                     const std::vector<ObsDecisionBound>& obs_low_points,
                     bool if_virtual, bool if_vehicle, bool if_bump,
                     double upper_limit, double target_speed,
                     double target_acc) {
  const auto& speed_iter_deduction_config = config::PlanningConfig::Instance()
                                                ->planning_research_config()
                                                .speed_iter_deduction;
...
DeductionEgoStateSequence HybirdIDM::CalHybirdIDMProcess() {
...
CheckDeductionCollisionCheck::CheckDeductionCollisionCheck(
...
std::pair<double, int> CheckDeductionCollisionCheck::FindClosestPoint(
...
std::pair<bool, int> CheckDeductionCollisionCheck::CheckAndCalCollisionPoint() {
  const auto& speed_iter_deduction_config = config::PlanningConfig::Instance()
                                                ->planning_research_config()
                                                .speed_iter_deduction;
      ...
      if (i == num_points - 1) {
        ...
        if (!obs_is_virtual_) {
          collision_buffer_l_for_end =
              collision_buffer_l_ +
              CalCollisionBuffer(ego_v, 0.3, obs_decision_.ObsSpeed()) +
              0.1 * ego_v;
