...
SpeedObsExtend::SpeedObsExtend(double id, double start_t, double end_t,
                               double start_s_l, double start_s_u,
                               double end_s_l, double end_s_u, bool if_reverse,
...
std::pair<double, int> SpeedObsExtend::FindClosestPoint(
...
// 耦合太严重了，只能在这替换数据层了
std::vector<ObsDecisionBound> SpeedObsExtend::InterpolatePoint(
...
ObsBoundPolySeris SpeedObsExtend::IterExtendStart() {
  const auto& speed_iter_deduction_config = config::PlanningConfig::Instance()
                                                ->planning_research_config()
                                                .speed_iter_deduction;
...
std::vector<ObsDecisionBound> SpeedObsExtend::InterpolatePointsForLow() {
  return InterpolatePoint(start_t_, start_s_l_, end_t_, end_s_l_, true);
}

std::vector<ObsDecisionBound> SpeedObsExtend::InterpolatePointsForUpper() {
  return InterpolatePoint(start_t_, start_s_u_, end_t_, end_s_u_, false);
}

void SpeedObsExtend::ResetExtendOrder() { extend_ = false; }
}  // namespace planning
}  // namespace neodrive
