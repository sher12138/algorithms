#include "speed_lane_borrow_decider.h"
...
SpeedLaneBorrowDecider::SpeedLaneBorrowDecider() {
  name_ = "SpeedLaneBorrowDecider";
}

SpeedLaneBorrowDecider::~SpeedLaneBorrowDecider() { Reset(); }

void SpeedLaneBorrowDecider::SaveTaskResults(TaskInfo& task_info) {
...
ErrorCode SpeedLaneBorrowDecider::Execute(TaskInfo& task_info) {
...
bool SpeedLaneBorrowDecider::Init(TaskInfo& task_info) {
  ...
  auto& borrow_lane_prepare_collision_info =
      task_info.current_frame()
          ...
          ->speed_obstacle_context.borrow_lane_prepare_collision_info;
...
bool SpeedLaneBorrowDecider::Process(TaskInfo& task_info) {
  ...
  bool is_prepare = (data_center_->master_info().lane_borrow_context().stage ==
...
bool SpeedLaneBorrowDecider::DynamicObsCheck(
  ...
  for (const auto* obs : dynamic_obs_vec) {
    ...
    Boundary boundary = obs->PolygonBoundary();
    if (reverse) {
      if (boundary.end_s() < adc_boundary.start_s()) {
        LOG_INFO("skip reverse back obstacle");
        continue;
      }
      double s_dis = boundary.start_s() - adc_boundary.end_s();
      min_relative_time = s_dis / (project_vel + adc_current_v_);
      if (min_relative_time > config_.ttc || min_relative_time < 0.0) {
        LOG_INFO("skip reverse safe obstacle");
        continue;
      }
      collide_with_path = true;
    } else {
      if (boundary.end_s() < adc_boundary.start_s()) {
        double s_dis = adc_boundary.start_s() - boundary.end_s();
        min_relative_time = s_dis / (project_vel - adc_current_v_);
        if (min_relative_time > config_.ttc || min_relative_time < 0.0) {
          LOG_INFO("skip back slow obstacle");
          continue;
        }
        collide_with_path = true;
      }
      if (boundary.start_s() > adc_boundary.end_s()) {
        double s_dis = boundary.start_s() - adc_boundary.end_s();
        min_relative_time = s_dis / (adc_current_v_ - project_vel);
        if (min_relative_time > config_.ttc || min_relative_time < 0.0) {
          LOG_INFO("skip front fast obstacle");
          continue;
        }
        collide_with_path = true;
      }
    ...
    if (collide_with_path) {
      double collide_s = obs->center_sl().s() + project_vel * min_relative_time;
      ReferencePoint ref_pt{};
      task_info.reference_line()->GetNearestRefPoint(collide_s, &ref_pt);
      SLPoint predict_obs_pt{collide_s, obs->center_sl().l()};
      Vec2d predict_xy_pt{};
      if (!task_info.reference_line()->GetPointInCartesianFrame(
              predict_obs_pt, &predict_xy_pt)) {
        continue;
      }
      auto obs_polygon = Utility::get_trajectory_point_polygon(
          obs->center(), predict_xy_pt, obs->velocity_heading(),
          ref_pt.heading(), obs->polygon());
...
bool SpeedLaneBorrowDecider::GetCollisionInfoFromCollisionCheck(
...
bool SpeedLaneBorrowDecider::IgnoreObsByObsSpeed(TaskInfo& task_info,
                                                 const Obstacle* obs) {
...
bool SpeedLaneBorrowDecider::IgnoreObsByObsHeading(TaskInfo& task_info,
                                                   const Obstacle* obs) {
...
bool SpeedLaneBorrowDecider::IgnoreObsByLateralDistance(TaskInfo& task_info,
                                                        const Obstacle* obs) {
...
bool SpeedLaneBorrowDecider::IgnoreObsByBackBoundary(TaskInfo& task_info,
                                                     const Obstacle* obs) {
  const auto& boundary = obs->PolygonBoundary();
...
bool SpeedLaneBorrowDecider::IsDynamicObsNeedIgnord(TaskInfo& task_info,
                                                    const Obstacle* obs,
                                                    bool& reverse,
                                                    double& project_v) {
  reverse = false;
...
bool SpeedLaneBorrowDecider::CalcBorrowBoundsInfo(TaskInfo& task_info) {
...
bool SpeedLaneBorrowDecider::PathCrossLaneBound(TaskInfo& task_info) {
...
bool SpeedLaneBorrowDecider::BorrowRegionCollideCheck(
...
bool SpeedLaneBorrowDecider::PrepareStatus(TaskInfo& task_info) {
...
bool SpeedLaneBorrowDecider::STDecision(TaskInfo& task_info) {
  auto& collision_info =
      task_info.current_frame()
          ->mutable_outside_planner_data()
          ->speed_obstacle_context.borrow_lane_prepare_collision_info;
...
std::size_t SpeedLaneBorrowDecider::PathIndex(TaskInfo& task_info,
                                              const double s) {
...
bool SpeedLaneBorrowDecider::CreateVirtualObstacle(TaskInfo& task_info) {
...
bool SpeedLaneBorrowDecider::CalPathZeroKappaEndS(TaskInfo& task_info,
                                                  double& expand_s) {
