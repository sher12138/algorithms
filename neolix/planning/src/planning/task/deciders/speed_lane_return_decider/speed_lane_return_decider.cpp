#include "speed_lane_return_decider.h"
...
std::size_t PathIndex(TaskInfo& task_info, const double s) {
  ...
  for (std::size_t i = 1; i < path_points.size(); i++) {
    ...
    auto& last_point = path_points[i - 1];
...
SpeedLaneReturnDecider::SpeedLaneReturnDecider() {
  name_ = "SpeedLaneReturnDecider";
}

SpeedLaneReturnDecider::~SpeedLaneReturnDecider() { Reset(); }

ErrorCode SpeedLaneReturnDecider::Execute(TaskInfo& task_info) {
...
bool SpeedLaneReturnDecider::Init(TaskInfo& task_info) {
  ...
  if (task_info.current_frame()->mutable_planning_data() == nullptr) {
    LOG_ERROR("invalid planning data.");
    return false;
  }
  if (task_info.current_frame()
          ->mutable_planning_data()
          ->mutable_decision_data() == nullptr) {
    LOG_ERROR("invalid decision data.");
    return false;
  }
  ...
  adc_current_v_ = inside_planner_data.vel_v;
  ...
  auto& borrow_lane_return_collision_info =
      task_info.current_frame()
          ->mutable_outside_planner_data()
          ->speed_obstacle_context.borrow_lane_return_collision_info;
  borrow_lane_return_collision_info.clear();

  stop_flag_ = false;
  return true;
}

bool SpeedLaneReturnDecider::Process(TaskInfo& task_info) {
  if (data_center_->master_info().curr_scenario() != ScenarioState::DETOUR) {
  ...
  if (task_info.current_frame()
          ...
          .dynamic_obstacle()
          .empty()) {
    LOG_INFO("dynamic obstacles empty, skip.");
    return true;
  }
  if (data_center_->master_info().lane_borrow_context().stage !=
          DetourStageState::BORROWING ||
      data_center_->master_info().lane_borrow_context().stage !=
          DetourStageState::REVERSE_LANE_BORROWING) {
    LOG_INFO("is not borrowing lane stage, skip.");
    return true;
  }
  ...
  /// check whether return status
  if (!ReturnStatus(task_info)) {
    LOG_INFO("is not borrowing return origin lane stage, skip.");
    return true;
  }
...
bool SpeedLaneReturnDecider::ReturnStatus(TaskInfo& task_info) {
  if (has_collision_risk) {
    has_collision_risk = false;
    LOG_INFO("Last frame has collision risk");
    return true;
  }
  const auto& path_points = task_info.current_frame()
                                ->outside_planner_data()
  ...
  for (std::size_t i = 5; i < path_points.size(); i += 5) {
    if (segment_dis_sum > 20.0) break;
    auto& curr_pt = path_points[i - 5];
    auto& forward_pt = path_points[i];
  ...
  } else if (data_center_->master_info().lane_borrow_context().borrow_side ==
  ...
  for (const auto& path : path_segments) {
    for (const auto& lane_segment : borrow_lane_bound_segments_) {
      if (path.has_intersect(lane_segment)) {
        cross_flag = true;
        break;
      }
    }
  ...
  for (auto& sl : frenet_path_points) {
    if (sl.s() - frenet_path_points.front().s() >
  ...
  LOG_INFO("adc_on_borrow_line[{}], cross_flag[{}], path_direction_flag[{}]",
           adc_on_borrow_lane, cross_flag, path_direction_flag);
  return adc_on_borrow_lane && cross_flag && path_direction_flag;
}

bool SpeedLaneReturnDecider::CalcBorrowBoundsInfo(TaskInfo& task_info) {
  const auto& reference_line = task_info.reference_line();
  std::vector<Vec2d> ref_points{};
  std::vector<double> ref_heading{};
  for (size_t i = 0; i < reference_line->ref_points().size(); i += 10) {
    ...
    ref_heading.emplace_back(point.heading());
    if (data_center_->master_info().lane_borrow_context().borrow_side ==
        LaneBorrowContext::BorrowSide::Left) {
      borrow_attention_lane_bounds_.emplace_back(point.left_lane_bound());
      borrow_attention_bounds_.emplace_back(-point.right_lane_bound());
    } else if (data_center_->master_info().lane_borrow_context().borrow_side ==
               LaneBorrowContext::BorrowSide::Right) {
      borrow_attention_lane_bounds_.emplace_back(-point.right_lane_bound());
      borrow_attention_bounds_.emplace_back(point.left_lane_bound());
    }
  }
  ...
  for (size_t i = 0; i < borrow_attention_s_vec_.size(); ++i) {
    lane_bound_xy_points.emplace_back(
        SLAnalyticTransformation::calculate_xypoint(
            ref_heading[i], ref_points[i], borrow_attention_lane_bounds_[i]));
    bound_xy_points.emplace_back(SLAnalyticTransformation::calculate_xypoint(
        ref_heading[i], ref_points[i], borrow_attention_bounds_[i]));
  }
  for (size_t i = 1; i < lane_bound_xy_points.size(); ++i) {
...
bool SpeedLaneReturnDecider::PathCrossLaneBound(TaskInfo& task_info) {
  const auto& path_points = task_info.current_frame()
                                ->outside_planner_data()
                                .path_data->path()
                                .path_points();
  bool flag_path_cross_lane_bound{false};
  for (size_t i = 0; i < path_points.size(); i += 5) {
    auto& veh_pt = path_points[i];
    Box2d veh_box = VehicleParam::Instance()->get_adc_bounding_box(
        {veh_pt.x(), veh_pt.y()}, veh_pt.theta(), 0.2, 0.2, 0.2);
    for (const auto& segment : borrow_lane_bound_segments_) {
...
bool SpeedLaneReturnDecider::DynamicObsCheck(
  ...
  for (const auto* obs : dynamic_obs_vec) {
    ...
    std::size_t min_s_index{adc_boundaries.size() + 10};
    ...
    for (std::size_t i = 0; i < pre_traj.trajectory_points().size(); i += 3) {
      ...
      auto obs_polygon = Utility::get_trajectory_point_polygon(
          ...
          obs->polygon());
      for (std::size_t j = 0; j < adc_boundaries.size(); j += 5) {
        auto ego_boundary = adc_boundaries[j];
    ...
    if (collide_with_path) {
      ...
      std::size_t min_s_index_clamp = std::max(
          ...
          std::min(
              static_cast<std::size_t>(adc_boundaries.size() - 1),
              std::min(static_cast<std::size_t>(path_accumulated_s.size() - 1),
                       min_s_index)));
      auto adc_collide_polygon = Polygon2d(adc_boundaries[min_s_index_clamp]);
...
bool SpeedLaneReturnDecider::STDecision(TaskInfo& task_info) {
  auto& collision_info =
      task_info.current_frame()
          ->mutable_outside_planner_data()
          ->speed_obstacle_context.borrow_lane_return_collision_info;
  ...
  if (collision_info.empty()) {
    LOG_INFO("borrow_lane__return_collision_info is empty, skip.");
    return true;
  }
  ...
  for (auto& info : collision_info) {
    ...
    } else if (borrow_side == LaneBorrowContext::BorrowSide::Right) {
      first_lateral_error =
  ...
  LOG_INFO("lane_borrow_return_collision_info size: {}", collision_info.size());
  for (const auto& info : collision_info) {
    ...
    std::size_t stop_index =
        std::max(static_cast<std::size_t>(0),
                 static_cast<std::size_t>(PathIndex(task_info, info.s)));
    if (std::abs(path_points[stop_index].kappa()) < 0.01) {
      continue;
    }
    stop_flag_ = true;
...
bool SpeedLaneReturnDecider::CreateVirtualObstacle(TaskInfo& task_info) {
...
bool SpeedLaneReturnDecider::IsDynamicObsNeedIgnord(TaskInfo& task_info,
                                                    const Obstacle* obs,
                                                    bool& reverse,
                                                    double& project_v) {
  reverse = false;
...
bool SpeedLaneReturnDecider::IgnoreObsByObsSpeed(TaskInfo& task_info,
                                                 const Obstacle* obs) {
...
bool SpeedLaneReturnDecider::IgnoreObsByBackBoundary(TaskInfo& task_info,
                                                     const Obstacle* obs) {
  const auto& boundary = obs->PolygonBoundary();
...
bool SpeedLaneReturnDecider::IgnoreObsByObsHeading(TaskInfo& task_info,
                                                   const Obstacle* obs) {
...
bool SpeedLaneReturnDecider::IgnoreObsByLateralDistance(TaskInfo& task_info,
                                                        const Obstacle* obs) {
...
bool SpeedLaneReturnDecider::GetCollisionInfoFromCollisionCheck(
    TaskInfo& task_info, const Obstacle* const obs, bool reverse,
    double project_vel,
    std::vector<LaneBorrowReturnCollisionInfo>& collision_info) {
  ...
  const auto& adc_boundaries = task_info.current_frame()
                                   ->outside_planner_data()
                                   .speed_obstacle_context.adc_boundaries;
  for (const auto& decision : dynamic_obstacles_decision) {
    if (decision.obstacle.id() == obs->id()) {
      has_collision_info = true;
      if (!decision.lower_points.empty()) {
        auto obs_collide_polygon = reverse ? decision.obstalce_polygons.back()
                                           : decision.obstalce_polygons.front();
        Boundary obs_collide_boundary{};
        if (!ref_line_util::ComputePolygonBoundary(task_info.reference_line(),
                                                   obs_collide_polygon,
        ...
        std::size_t lower_adc_first_index_clamp =
            std::max(0, std::min(static_cast<int>(adc_boundaries.size() - 1),
                                 decision.lower_adc_first_index));
        auto adc_collide_polygon =
            Polygon2d(adc_boundaries[lower_adc_first_index_clamp]);
        Boundary adc_collide_boundary{};
        if (!ref_line_util::ComputePolygonBoundary(task_info.reference_line(),
                                                   adc_collide_polygon,
                                                   &adc_collide_boundary)) {
...
bool SpeedLaneReturnDecider::BorrowRegionCollideCheck(
...
void SpeedLaneReturnDecider::SaveTaskResults(TaskInfo& task_info) {
  if (update_limited_speed_) {
    neodrive::global::planning::SpeedLimit internal_speed_limit{};
    internal_speed_limit.set_source_type(SpeedLimitType::LANE_BORROW);
    ...
    LOG_INFO(
        "LANE_BORROW Exit {} limit speed: speed = {:.2f}, acc = {:.2f}",
