#include "speed_reversed_obs_decider.h"
...
SpeedReversedObsDecider::SpeedReversedObsDecider() {
  name_ = "SpeedReversedObsDecider";
}

SpeedReversedObsDecider::~SpeedReversedObsDecider() {}

ErrorCode SpeedReversedObsDecider::Execute(TaskInfo& task_info) {
...
void SpeedReversedObsDecider::SaveTaskResults(TaskInfo& task_info) {
  if (update_limited_speed_) {
    ...
    dynamic_obs_caution_infos_ptr->emplace_back("Reverse", limited_speed_,
...
void SpeedReversedObsDecider::Reset() {
...
bool SpeedReversedObsDecider::Init(TaskInfo& task_info) {
  Reset();
  if (!update_limited_speed_) {
    last_limited_speed_ =
  ...
  dynamic_obs_caution_infos_ptr =
      &task_info.current_frame()
           ->mutable_outside_planner_data()
           ->speed_obstacle_context.dynamic_obs_caution_infos;
  if (!dynamic_obs_caution_infos_ptr) {
    return false;
  }
...
bool SpeedReversedObsDecider::InitRoadBoundaries(TaskInfo& task_info) {
  const auto& speed_reversed_repulsive_field_config =
      config::PlanningConfig::Instance()
          ->planning_research_config()
          .speed_reversed_repulsive_field_caution;
...
bool SpeedReversedObsDecider::InitSingleRoadBoundaries(TaskInfo& task_info) {
  const auto& speed_reversed_obs_config =
      config::PlanningConfig::Instance()->plan_config().speed_reversed_obs;
 ...
 * @return false
 */
bool SpeedReversedObsDecider::Process(TaskInfo& task_info) {
  const auto& adc_front_road_boundaries =
      task_info.current_frame()
          ->outside_planner_data()
          .speed_obstacle_context.adc_front_road_boundaries;
  ...
  SpeedReversedObsContextInfo(adc_front_road_boundaries,
                              dp_st_map_ignore_dynamic_obs_id);
  return true;
}

// check obs with no decision ,use repulsive field method
void SpeedReversedObsDecider::GetSpeedLimitByRepulsiveFieldForReversedObs(
    TaskInfo& task_info) {
  // obs has no decision
...
void SpeedReversedObsDecider::CalcSpeedLimitbyRepulsiveField(
    TaskInfo& task_info) {
  if (reverse_use_repulsive_obs_.size() == 0) {
    return;
  }
 ...
 */
void SpeedReversedObsDecider::GetSpeedLimitByReversedObs(TaskInfo& task_info) {
  const auto& speed_reversed_obs_config =
      config::PlanningConfig::Instance()->plan_config().speed_reversed_obs;
  const auto& dynamic_obstacles_decision =
      task_info.current_frame()
  ...
  for (auto& obs_decision : dynamic_obstacles_decision) {
    ...
    CheckBeforeComeInSingleRoad(obstacle, dp_st_map_ignore_dynamic_obs_id);
    ...
    // dp_st_map_ignore_dynamic_obs_id.insert(obstacle.id());
...
// 2as=v0^2-v^2
double SpeedReversedObsDecider::CalDecelerationByObs(
 ...
 * @return double
 */
double SpeedReversedObsDecider::GetSpeedAtEmergencyPoint(
...
void SpeedReversedObsDecider::CheckBeforeComeInSingleRoad(
...
void SpeedReversedObsDecider::AdcSlowDown(double distance_to_stop) {
  update_limited_speed_ = true;
...
void SpeedReversedObsDecider::ReversedObsContextInfo(const Obstacle& obstacle) {
  LOG_INFO(
...
void SpeedReversedObsDecider::SpeedReversedObsContextInfo(
