#include "speed_stop_to_go_freespace_decider.h"
...
SpeedStopToGoFreespaceDecider::SpeedStopToGoFreespaceDecider() {
  name_ = "SpeedStopToGoFreespaceDecider";
}

SpeedStopToGoFreespaceDecider::~SpeedStopToGoFreespaceDecider() { Reset(); }

ErrorCode SpeedStopToGoFreespaceDecider::Execute(TaskInfo& task_info) {
...
void SpeedStopToGoFreespaceDecider::SaveTaskResults(TaskInfo& task_info) {
...
void SpeedStopToGoFreespaceDecider::UpdatedLimitedSpeed() {
...
bool SpeedStopToGoFreespaceDecider::Init(TaskInfo& task_info) {
...
bool SpeedStopToGoFreespaceDecider::DataCheck(TaskInfo& task_info) {
...
bool SpeedStopToGoFreespaceDecider::Process(TaskInfo& task_info) {
  if (!Init(task_info)) {
...
void SpeedStopToGoFreespaceDecider::GetAdcCheckPolygons(TaskInfo& task_info) {
  ...
  const auto& path_accumulated_s =
      outside_data->speed_obstacle_context.path_accumulated_s;
...
bool SpeedStopToGoFreespaceDecider::SegmentCutByPolygon(
...
void SpeedStopToGoFreespaceDecider::ProcessFreespaceCollisionRisk(
  ...
  if (obs_keep_cnt_ > 0) {
    ...
    LOG_INFO(
        "limit speed and create stop to go freespace virtual obstacle, init_s, "
        "adc_front_edge_s_:{:.2f}, {:.2f}",
