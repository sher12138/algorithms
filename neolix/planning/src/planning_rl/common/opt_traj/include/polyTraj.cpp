...
int PolyTraj::generate_5th_order(double x0, double x0_dot, double x0_ddot,
                                 double x1, double x1_dot, double x1_ddot,
                                 double /*t_current*/,
                                 double deltaT,  // TODO: remove t_current?!?
                                 double horizon) {
  if (deltaT < 0.0001) {
    return -1;
  }
  ...
  coeff_[3] = (x1_ddot * t2 - 3.0 * x0_ddot * t2 - 8.0 * x1_dot * t -
               12.0 * x0_dot * t + 20.0 * x1 - 20.0 * x0) /
              (2.0 * t3);
  coeff_[4] = (-2.0 * x1_ddot * t * t + 3.0 * x0_ddot * t2 + 14.0 * x1_dot * t +
               16.0 * x0_dot * t - 30.0 * x1 + 30.0 * x0) /
              (2.0 * t4);
  coeff_[5] = (x1_ddot * t2 - x0_ddot * t2 - 6.0 * x1_dot * t -
               6.0 * x0_dot * t + 12.0 * x1 - 12.0 * x0) /
              (2.0 * t5);
  te = t;
...
int PolyTraj::generate_4th_order(double x0, double x0_dot, double x0_ddot,
  ...
  // printf("x0, x0_dot, x0_ddot, x1_dot, x1_ddot, t, t_hor = %f, %f, %f, %f,
  ...
  coeff_[1] = x0_dot;
  coeff_[2] = x0_ddot / 2.0;
  coeff_[3] = -(coeff_[1] - x1_dot + 2.0 * coeff_[2] * t) / t2 -
              (x1_ddot - 2.0 * coeff_[2]) / (3.0 * t);
  coeff_[4] = (coeff_[1] - x1_dot + 2.0 * coeff_[2] * t) / (2.0 * t3) +
              (x1_ddot - 2.0 * coeff_[2]) / (4.0 * t2);
  coeff_[5] = 0.0;
...
int PolyTraj::eval_trajectory(const double& t,
...
void PolyTraj::sampleArgumentEquidistant(const double& t_current,
                                         double resolution) {
...
void PolyTraj::sampleArgumentSynchronized(
    const std::vector<std::vector<double> >& sample_ref) {
  // Samples the trajectory at the same discrete arguments as the reference
  // vectors
  ...
  for (it = sample_ref.begin(), it_traj = discrete_traj_points_.begin();
       it != sample_ref.end(); it++, it_traj++) {
    std::vector<double>::const_iterator it_vec;
    for (it_vec = it->begin(); it_vec != it->end(); it_vec++) {
      trajectory_point_1D traj_point;
...
void PolyTraj::set_debug_info(const double& arg, const double& jerk,
                              const double& deviation, const double& cost_arg,
                              const double& cost_jerk,
                              const double& cost_deviation) {
  arg_ = arg;
  jerk_ = jerk;
...
void PolyTraj::get_debug_info(double& arg, double& jerk, double& deviation,
                              double& cost_arg, double& cost_jerk,
                              double& cost_deviation) const {
  arg = arg_;
  jerk = jerk_;
