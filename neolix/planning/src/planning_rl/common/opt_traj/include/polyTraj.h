...
class PolyTraj {
  ...
  movement_state end_state;  // between 5th order and 3rd order polynomial for
                             // fast calcualtion
  double total_cost;
 ...
 public:
  ...
  int generate_5th_order(double x0, double x0_dot, double x0_ddot, double x1,
                         double x1_dot, double x1_ddot, double t_current,
                         double t, double horizon);
  ...
  void set_debug_info(const double& arg, const double& jerk,
                      const double& deviation, const double& cost_arg,
                      const double& cost_jerk, const double& cost_deviation);
  void get_debug_info(double& arg, double& jerk, double& deviation,
                      double& cost_arg, double& cost_jerk,
                      double& cost_deviation) const;
  movement_state get_end_state() { return end_state; }

  void sampleArgumentEquidistant(const double& t_current,
                                 double time_resolution);
  void sampleArgumentSynchronized(
