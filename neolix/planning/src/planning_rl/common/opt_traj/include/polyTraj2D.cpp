...
PolyTraj2D::PolyTraj2D(
    const std::vector<CurvePoint>& /*center_line*/, const PolyTraj& traj_lat,
  ...
  generation_mode_ = mode;  // either arclength-based for slow speeds or time
                            // base for higher speeds
  ...
  int number_of_points_long =
      root_traj.discrete_traj_points_[0].size();  // long does not

  trajectory2D_.reserve(number_of_points_long);
  std::vector<trajectory_point_1D>::const_iterator it_root_traj_points,
      it_root_, it_lat_trajectory_points;
  ...
  for (it_root_traj_points = root_traj.discrete_traj_points_[0].begin(),
      ...
      it_lat_trajectory_points++) {
    TrajectoryPoint2D transformed_point;
    transformed_point = latLongTrajectories2GlobalCordinates(
        *it_lat_trajectory_points, *it_root_traj_points, *it_foot_curve_points);
    // Add new 2D point to vector
    trajectory2D_.push_back(transformed_point);
  }
}

PolyTraj2D::PolyTraj2D(const PolyTraj& traj_lat, const RootTraj& root_traj) {
  generation_mode_ = time_based;  // either arclength-based for slow speeds or
                                  // time base for higher speeds
  // memorize original lat/long trajectories for later use
  pt_traj_lat_ = &traj_lat;
  pt_traj_root_ = &root_traj;
  ...
  int number_of_points_long =
      root_traj.discrete_traj_points_[0].size();  // long does not
  // std::cout << number_of_points_long << std::endl;
  // std::cout << root_traj.foot_curve_points_.size() << std::endl;
  // std::cout << traj_lat.discrete_traj_points_[0].size() << std::endl;
  ...
  for (it_root_traj_points = root_traj.discrete_traj_points_[0].begin(),
      it_foot_curve_points = root_traj.foot_curve_points_.begin(),
      it_lat_trajectory_points = traj_lat.discrete_traj_points_[0].begin();
       it_root_traj_points != root_traj.discrete_traj_points_[0].end();
       it_root_traj_points++, it_foot_curve_points++,
      it_lat_trajectory_points++) {
    TrajectoryPoint2D transformed_point;
...
void PolyTraj2D::calc_extreme_values(const double t_delay) {
  ...
  t_start += t_delay;
  std::vector<TrajectoryPoint2D>::iterator it_tp;
  for (it_tp = trajectory2D_.begin(); it_tp != trajectory2D_.end(); it_tp++) {
    if (it_tp->t > t_start) {
      break;
    }
  }
  for (; it_tp != trajectory2D_.end(); it_tp++) {
    // curvature
...
TrajectoryPoint2D PolyTraj2D::latLongTrajectories2GlobalCordinates(
    const trajectory_point_1D& lat_traj_point,
    const trajectory_point_1D& long_traj_point,
    const CurvePoint& foot_curve_point) const {
  TrajectoryPoint2D traj_point_res;
  ...
  if ((-DBL_EPSILON < one_minus_kappa_d) && (one_minus_kappa_d < DBL_EPSILON)) {
    throw printf(
        "The calculated trajectory is too far off from center line for its "
        "curvature!");
  }
  ...
  if (generation_mode_ == time_based) {
    ...
    // We restrict the trajectories with high speed generation to be aligned to
    // the center line at the end
    ...
    double expr_square =
        d_ddot_minus_d_prime_s_ddot * d_ddot_minus_d_prime_s_ddot;
    if (s_dot > MIN_VEL) {
      d_pprime = d_ddot_minus_d_prime_s_ddot / s_dot_square;
    } else if (expr_square > MIN_DPRIME_SQUARE) {
      d_pprime =
          DBL_MAX;  // set curvature to infinity so that we won't use trajectory
    } else {
      d_pprime = 0.0;  // we assume that car drives parallel to centerline
    }
  } else {  // generation mode == arclength_based   -> take d_prime and d_pprime
  ...
  delta_theta_x = delta_theta;  // memorize for filtering the trajectories that
                                // are too far off
  if ((delta_theta <= -M_PI_2) || (M_PI_2 <= delta_theta)) {
    throw printf("The calculated heading is too far off from the center line!");
  }
  ...
  const double one_minus_kappa_d_div_cos_delta_theta =
      one_minus_kappa_d * one_over_cos_delta_theta;
  const double cos_delta_theta_div_one_minus_kappa_d =
      1.0 / one_minus_kappa_d_div_cos_delta_theta;
  ...
  kappa_x = (d_pprime + kappa_times_d__prime * tan_delta_theta) *
                cos_delta_theta * cos_delta_theta_div_one_minus_kappa_d *
                cos_delta_theta_div_one_minus_kappa_d +
  ...
  const double delta_theta_prime =
      one_minus_kappa_d_div_cos_delta_theta * kappa_x - kappa;

  a_x = s_ddot * one_minus_kappa_d_div_cos_delta_theta +
        s_dot_square * one_over_cos_delta_theta *
            (one_minus_kappa_d * tan_delta_theta * delta_theta_prime -
...
/*static*/ void PolyTraj2D::globalCordinates2LatLong(
    const TrajectoryPoint2D& tp_2d, const std::vector<CurvePoint>& center_line,
  ...
  if (center_line.size() <= 1) {
    throw printf("No entries in center line!");
  }
  ...
  for (it_cl_point = center_line.begin(); it_cl_point != center_line.end();
       it_cl_point++) {
  ...
  try {
    projectionCondition(center_line, s0, tp_2d.x, tp_2d.y,
                        f0);  // f0 = f(s, x, y)
    projectionCondition(center_line, s1, tp_2d.x, tp_2d.y, f1);
  } catch (Ex<>& e) {
    throw printf("Secant method -> ");
  }
  ...
  do {
    ...
    if ((-DBL_EPSILON < delta_f) && (delta_f < DBL_EPSILON)) {
      throw printf("Secant method: too many iterations?");
    }
    s2 = s1 - (s1 - s0) / (f1 - f0) * f1;

    try {
      projectionCondition(center_line, s2, tp_2d.x, tp_2d.y,
                          f2);  // f2 = f(s, x, y)
    } catch (Ex<>& e) {
      throw printf("Secant method: f2 = f(s, x, y) -> ");
    }
    ...
    count--;
    if (count == 0) {
      throw printf("Secant method has not converged fast enough!");
    }
  ...
  if ((delta_theta >= M_PI_2) || (delta_theta <= -M_PI_2)) {
    printf("theta_r: %f, theta_x: %f, delta_theta: %f\n", theta_r, theta_x,
           delta_theta);
    throw printf("Projection failed! Reference heading is too far off!");
  }
  ...
  const double one_minus_kappa_d_div_cos_delta_theta =
      one_minus_kappa_d / cos_delta_theta;
  const double cos_delta_theta_div_one_minus_kappa_d =
      1.0 / one_minus_kappa_d_div_cos_delta_theta;
  const double delta_theta_prime =
      one_minus_kappa_d_div_cos_delta_theta * kappa_x - kappa_r;

  s_dot = v_x * cos_delta_theta_div_one_minus_kappa_d;  // s. paper
  const double s_dot_square = s_dot * s_dot;
  /*const*/ double d_prime_ =
      one_minus_kappa_d * tan_delta_theta;  // needed in either case
  const double kappa_times_d__prime =
      kappa_r_prime * d + kappa_r * d_prime_;  // needed in either case
  ...
  if (generation_mode == time_based) {
    ...
    d_ddot =
        a_x * sin_delta_theta +
        v_x * cos_delta_theta * (kappa_x * v_x - kappa_r * s_dot);  // s. paper
  ...
  s_ddot = (a_x - s_dot_square * one_div_cos_delta_theta *
                      (one_minus_kappa_d * tan_delta_theta * delta_theta_prime -
                       kappa_times_d__prime)) *
           cos_delta_theta_div_one_minus_kappa_d;
  // s. paper
...
/*static*/ void PolyTraj2D::projectionCondition(
    const std::vector<CurvePoint>& center_line, const double& s_foot,
  ...
  try {
    ...
    throw printf("Polynomial trajectory -> ");
  }
  // Function which should be zero
  f = (x1 - foot_point.x) * cos(foot_point.theta) +
      (x2 - foot_point.y) * sin(foot_point.theta);
  return;
}

void PolyTraj2D::calculateNextInitialStates(
    const double& t_next_deadline, movement_state& next_lat_state,
    movement_state& next_long_state) const {
  trajectory_point_1D lat_tp;
  trajectory_point_1D long_tp;
  double t_last_deadline = pt_traj_root_->discrete_traj_points_[0][0]
                               .arg;  // TODO: introduce new member var t_start
  if (t_last_deadline > t_next_deadline) {
    throw printf("t_last_deadline > t_next_deadline!");
  }
  evalLatLongTrajectoriesTimeBased(
      t_last_deadline, (t_next_deadline - t_last_deadline), lat_tp, long_tp);
...
TrajectoryPoint2D PolyTraj2D::evaluateTrajectoryAtT(const double& t) const {
  if (trajectory2D_.empty()) {
    throw printf("trajectory is empty");
  }

  const double& t_planning =
      trajectory2D_[0].t;  // // TODO: introduce new member var t_start
  trajectory_point_1D lat_tp, long_tp;

  // get data from lateral and longitudinal trajectory
  if (generation_mode_ == time_based) {
    evalLatLongTrajectoriesTimeBased(t_planning, (t - t_planning), lat_tp,
                                     long_tp);
  } else {  // generation_mode_ == arclength_based
    const double& s_start = pt_traj_root_->discrete_traj_points_[0][0]
                                .x;  // TODO: introduce new member var s_start
    evalLatLongTrajectoriesArclengthBased(t_planning, (t - t_planning), s_start,
                                          lat_tp, long_tp);
  }
  ...
  return latLongTrajectories2GlobalCordinates(lat_tp, long_tp,
                                              cp_interpol_foot);
}

TrajectoryPoint2D PolyTraj2D::calculateNextStartTrajectoryPoint(
    const double& t_next) const {
  return evaluateTrajectoryAtT(t_next);
}

void PolyTraj2D::evalLatLongTrajectoriesTimeBased(
    const double& t, const double& delta_t, trajectory_point_1D& lat_tp,
...
void PolyTraj2D::evalLatLongTrajectoriesArclengthBased(
    const double& t, const double& delta_t, const double& s,
    trajectory_point_1D& lat_tp, trajectory_point_1D& long_tp) const {
  /* t = relative time */
...
void PolyTraj2D::generateNewControlTrajectory(
  ...
  std::vector<TrajectoryPoint2D> new_traj;
  std::vector<TrajectoryPoint2D>::const_iterator it;
  ...
  if (trajectory.size() > 0) {
    for (it = trajectory.begin();
         ((it->t < t_planning) && (it != trajectory.end())); it++) {
      if (((it + 1)->t >
           t_current)  // keep last point to be able to interpolate
          || ((t_current < it->t) &&
              (it->t < t_planning))) {  // also keep points between now and next
                                        // planning step
  ...
  TrajectoryPoint2D tp_2d;

  double t_last_in_new_traj;
  if (new_traj.size() == 0) {
    t_last_in_new_traj =
        t_planning - control_t_res;  // thats the best you can do when you start
    // printf("first step\n");                       // minus control_t_res
    // because of for loop
  } else {
    t_last_in_new_traj = new_traj.back().t;
  }
  for (double t = t_last_in_new_traj + control_t_res;
       t <= (t_planning + t_control_horizon); t += control_t_res) {
...
std::ostream& operator<<(std::ostream& ostr, const PolyTraj2D& traj2d) {
  double arg_lat, jerk_lat, deviation_lat, cost_arg_lat, cost_jerk_lat,
      cost_deviation_lat;
  double arg_lon, jerk_lon, deviation_lon, cost_arg_lon, cost_jerk_lon,
      cost_deviation_lon;
  traj2d.pt_traj_lat_->get_debug_info(arg_lat, jerk_lat, deviation_lat,
                                      cost_arg_lat, cost_jerk_lat,
                                      cost_deviation_lat);
  traj2d.pt_traj_root_->get_debug_info(arg_lon, jerk_lon, deviation_lon,
                                       cost_arg_lon, cost_jerk_lon,
                                       cost_deviation_lon);
  ostr << "---------------------2D Trajectory Info "
          "----------------------------------------------"
       << std::endl;
  ostr << "total_costs=" << traj2d.total_cost_;
  ostr << "  mode_lat="
       << (traj2d.generation_mode_ == time_based ? "time_based"
                                                 : "arclength_based")
       << "  max_curvature=" << traj2d.get_max_curvature()
       << "  min_velocity=" << traj2d.get_min_velocity() << std::endl;
  ostr << "lat: "
       << "index=" << traj2d.index_lat_ << " arg=" << arg_lat
       << " jerk=" << jerk_lat << " deviation=" << deviation_lat
       << "  total_costs=" << traj2d.pt_traj_lat_->get_total_cost()
       << "  cost_arg=" << cost_arg_lat << "  cost_jerk=" << cost_jerk_lat
       << "  cost_dev=" << cost_deviation_lat << std::endl;
  ostr << "lon: "
       << "index=" << traj2d.index_lon_ << " arg=" << arg_lon
       << " jerk=" << jerk_lon << " deviation=" << deviation_lon
       << "  total_costs=" << traj2d.pt_traj_root_->get_total_cost()
       << "  cost_arg=" << cost_arg_lon << "  cost_jerk=" << cost_jerk_lon
       << "  cost_dev=" << cost_deviation_lon << std::endl;
  ostr << "--------------------------------------------------------------------"
          "------------------"
       << std::endl;
  return ostr;
