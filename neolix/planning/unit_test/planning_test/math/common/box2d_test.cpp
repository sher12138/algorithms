...
TEST(Box2dTest, DebugString) {
  Box2d box1({0, 0}, 0, 4, 2);
  Box2d box2({5, 2}, 0, 4, 2);
  EXPECT_EQ(box1.debug_string(),
            "box2d ( center = vec2d ( x = 0  y = 0 )  heading = 0  length = 4  "
            "width = 2 )");
  EXPECT_EQ(box2.debug_string(),
...
TEST(Box2dTest, has_overlap) {
  ...
  EXPECT_TRUE(box1.has_overlap(Segment2d({0, 0}, {1, 1})));
  EXPECT_TRUE(box1.has_overlap(Segment2d({0, 0}, {3, 3})));
  EXPECT_TRUE(box1.has_overlap(Segment2d({0, -3}, {0, 3})));
  EXPECT_TRUE(box1.has_overlap(Segment2d({4, 0}, {-4, 0})));
  EXPECT_TRUE(box1.has_overlap(Segment2d({-4, -4}, {4, 4})));
  EXPECT_TRUE(box1.has_overlap(Segment2d({4, -4}, {-4, 4})));
  EXPECT_FALSE(box1.has_overlap(Segment2d({-4, -4}, {4, -4})));
  EXPECT_FALSE(box1.has_overlap(Segment2d({4, -4}, {4, 4})));
}

TEST(Box2dTest, GetAABox) {
  AABox2d aabox1 = box1.get_aa_box();
  AABox2d aabox2 = box2.get_aa_box();
  AABox2d aabox3 = box3.get_aa_box();
  AABox2d aabox4 = box4.get_aa_box();
...
TEST(Box2dTest, distance_to) {
  ...
  EXPECT_NEAR(box1.distance_to(Segment2d({-4, -4}, {4, 4})), 0.0, 1e-5);
  EXPECT_NEAR(box1.distance_to(Segment2d({4, -4}, {-4, 4})), 0.0, 1e-5);
  EXPECT_NEAR(box1.distance_to(Segment2d({0, 2}, {4, 4})), 1.0, 1e-5);
  EXPECT_NEAR(box1.distance_to(Segment2d({2, 2}, {3, 1})), std::sqrt(2.0) / 2.0,
              1e-5);
}

TEST(Box2dTest, is_point_in) {
...
TEST(Box2dTest, is_point_on_boundary) {
...
TEST(Box2dTest, TestByRandom) {
  ...
  for (int iter = 0; iter < 100; ++iter) {
    ...
    for (int iter2 = 0; iter2 < 1000; ++iter2) {
      ...
      EXPECT_EQ(box.is_point_on_boundary({x, y}),
                poly.is_point_on_boundary({x, y}));
      ...
      const Segment2d segment({x, y}, {other_x, other_y});
