...
TEST(Polygon2dTest, polygon_is_point_in) {
  const Polygon2d poly1(Box2d::create_aa_box({0, 0}, {1, 1}));
  EXPECT_EQ(poly1.debug_string(),
...
TEST(Polygon2dTest, distance_toPoint) {
  const Box2d box1(Box2d::create_aa_box({0, 0}, {1, 1}));
...
TEST(Polygon2dTest, distance_toLineSegment) {
  const Polygon2d poly1(Box2d::create_aa_box({0, 0}, {1, 1}));
...
TEST(Polygon2dTest, distance_toPolygon) {
  const Polygon2d poly1(Box2d::create_aa_box({0, 0}, {1, 1}));
  const Polygon2d poly2({{0, 1}, {1, 0}, {0, -1}, {-1, 0}});
  const Polygon2d poly3(Box2d::create_aa_box({2, 2}, {3, 3}));
  const Polygon2d poly4(Box2d::create_aa_box({-10, -10}, {10, 10}));
...
TEST(Polygon2dTest, ContainPolygon) {
  const Polygon2d poly1(Box2d::create_aa_box({0, 0}, {3, 3}));
  const Polygon2d poly2(Box2d::create_aa_box({1, 1}, {2, 2}));
  const Polygon2d poly3(Box2d::create_aa_box({1.5, 1.5}, {4, 4}));
  const Polygon2d poly4(Box2d::create_aa_box({-10, -10}, {10, 10}));
  EXPECT_TRUE(poly1.is_contain(poly2));
  EXPECT_FALSE(poly2.is_contain(poly1));

  EXPECT_FALSE(poly1.is_contain(poly3));
  EXPECT_FALSE(poly2.is_contain(poly3));
  EXPECT_FALSE(poly3.is_contain(poly1));
  EXPECT_FALSE(poly3.is_contain(poly2));

  EXPECT_TRUE(poly4.is_contain(poly1));
  EXPECT_TRUE(poly4.is_contain(poly2));
  EXPECT_TRUE(poly4.is_contain(poly3));
  ...
  EXPECT_FALSE(poly5.is_contain(poly6));
  EXPECT_TRUE(poly5.is_contain(poly7));
  EXPECT_TRUE(poly5.is_contain(poly8));
  EXPECT_TRUE(poly5.is_contain(poly9));
}
...
TEST(Polygon2dTest, ConvexHull) {
  ...
  EXPECT_FALSE(Polygon2d::compute_convex_hull({{3, 4}, {3, 4}, {5, 6}, {5, 6}},
                                              &polygon));

  EXPECT_TRUE(
      Polygon2d::compute_convex_hull({{0, 0}, {0, 4}, {3, 0}}, &polygon));
  EXPECT_TRUE(polygon.is_convex());
  EXPECT_NEAR(6.0, polygon.area(), 1e-5);

  EXPECT_TRUE(Polygon2d::compute_convex_hull({{0, 0}, {0, 4}, {3, 0}, {3, 4}},
                                             &polygon));
  EXPECT_TRUE(polygon.is_convex());
...
TEST(Polygon2dTest, Overlap) {
  const Polygon2d poly1(Box2d::create_aa_box({0, 0}, {2, 2}));
  const Polygon2d poly2(Box2d::create_aa_box({1, 1}, {3, 3}));
  const Polygon2d poly3(Box2d::create_aa_box({2, 0}, {4, 2}));
  ...
  EXPECT_NEAR(poly1.compute_IoU(poly2), 0.1428, 1e-4);
  EXPECT_NEAR(poly2.compute_IoU(poly1), 0.1428, 1e-4);

  EXPECT_NEAR(poly1.compute_IoU(poly3), 0.0, 1e-4);
  EXPECT_NEAR(poly3.compute_IoU(poly1), 0.0, 1e-4);

  EXPECT_NEAR(poly1.compute_IoU(poly4), 0.0909, 1e-4);
  EXPECT_NEAR(poly4.compute_IoU(poly1), 0.0909, 1e-4);

  EXPECT_NEAR(poly2.compute_IoU(poly3), 0.1428, 1e-4);
  EXPECT_NEAR(poly3.compute_IoU(poly2), 0.1428, 1e-4);

  EXPECT_NEAR(poly2.compute_IoU(poly4), 0.5, 1e-4);
  EXPECT_NEAR(poly4.compute_IoU(poly2), 0.5, 1e-4);

  EXPECT_NEAR(poly3.compute_IoU(poly4), 0.0909, 1e-4);
  EXPECT_NEAR(poly4.compute_IoU(poly3), 0.0909, 1e-4);

  Vec2d first_intersect;
  Vec2d last_intersect;
  EXPECT_FALSE(poly1.get_overlap(Segment2d({-1, 0}, {-1, 2}), &first_intersect,
                                 &last_intersect));
  EXPECT_FALSE(poly1.get_overlap(Segment2d({-1, 1}, {-3, 1}), &first_intersect,
                                 &last_intersect));
  EXPECT_FALSE(poly1.get_overlap(Segment2d({1, 3}, {1, 5}), &first_intersect,
                                 &last_intersect));

  EXPECT_TRUE(poly1.get_overlap(Segment2d({1, -1}, {1, 3}), &first_intersect,
                                &last_intersect));
  ...
  EXPECT_TRUE(poly1.get_overlap(Segment2d({1, -1}, {1, 1}), &first_intersect,
                                &last_intersect));
  EXPECT_NEAR(1.0, first_intersect.x(), 1e-5);
  EXPECT_NEAR(0.0, first_intersect.y(), 1e-5);
  EXPECT_NEAR(1.0, last_intersect.x(), 1e-5);
  ...
  EXPECT_TRUE(poly1.get_overlap(Segment2d({1, 3}, {3, 1}), &first_intersect,
                                &last_intersect));
  ...
  EXPECT_FALSE(poly1.get_overlap(Segment2d({4, 3}, {4, 3}), &first_intersect,
                                 &last_intersect));
  EXPECT_TRUE(poly1.get_overlap(Segment2d({1, 1}, {1, 1}), &first_intersect,
                                &last_intersect));
  EXPECT_NEAR(1.0, first_intersect.x(), 1e-5);
  ...
  std::vector<Segment2d> overlap_line_segments =
      poly5.get_all_overlaps(Segment2d({-10, 1.5}, {10, 1.5}));
  ...
  overlap_line_segments = poly5.get_all_overlaps(Segment2d({-10, 1}, {10, 1}));
  ...
  overlap_line_segments =
      poly5.get_all_overlaps(Segment2d({-10, 0.5}, {10, 0.5}));
  EXPECT_EQ(1, overlap_line_segments.size());
  EXPECT_NEAR(0.0, overlap_line_segments[0].start().x(), 1e-5);
  ...
  overlap_line_segments =
      poly5.get_all_overlaps(Segment2d({-10, 2.5}, {10, 2.5}));
  EXPECT_EQ(0, overlap_line_segments.size());

  overlap_line_segments = poly5.get_all_overlaps(Segment2d({2, 0.5}, {2, 0.5}));
  EXPECT_EQ(1, overlap_line_segments.size());
  EXPECT_NEAR(2.0, overlap_line_segments[0].start().x(), 1e-5);
  EXPECT_NEAR(0.5, overlap_line_segments[0].start().y(), 1e-5);
  EXPECT_NEAR(2.0, overlap_line_segments[0].end().x(), 1e-5);
  EXPECT_NEAR(0.5, overlap_line_segments[0].end().y(), 1e-5);
  overlap_line_segments = poly5.get_all_overlaps(Segment2d({5, 0.5}, {5, 0.5}));
  EXPECT_EQ(0, overlap_line_segments.size());
  ...
  for (int iter = 0; iter < 10000; ++iter) {
    ...
    for (const auto &line_segment1 : polygon1.segments()) {
      for (const auto &line_segment2 : polygon2.segments()) {
        Vec2d pt;
...
TEST(Polygon2dTest, BoundingBox) {
  Polygon2d poly1(Box2d::create_aa_box({0, 0}, {2, 2}));
  ...
  EXPECT_TRUE(Polygon2d(box).is_contain(poly1));
  AABox2d aabox = poly1.aa_bounding_box();
  ...
  EXPECT_NEAR(4.0, box.area(), 1e-5);
  EXPECT_TRUE(Polygon2d(box).is_contain(poly2));
  aabox = poly2.aa_bounding_box();
  ...
  for (int iter = 0; iter < 1000; ++iter) {
    ...
    for (int iter2 = 0; iter2 < 10; ++iter2) {
      ...
      EXPECT_TRUE(Polygon2d(box).is_contain(polygon));
...
TEST(Polygon2dTest, Expand) {
  {
    const Polygon2d poly(Box2d::create_aa_box({0, 0}, {2, 2}));
